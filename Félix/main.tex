...
Thus the products $[xy]$ satisfies all the conditions on the product in a Lie algebra. The Lie algebra obtained in this way is called \textit{the Lie algebra} of the associative algebra $\mathfrak{U}$. We shall denote this Lie algebra as $\mathfrak{U}_L$.
...
\begin{enumerate}
    ...
    \item From Jacobson,\cite{jacobson2013lie}
    ...
    Expending this sum we get the results wanted
    ...
    Given $\Vec{a}$, we can consider that $\mathfrak{L}(\Vec{a})$ is generated by the set $X=\{x_{(i,j)}: 1 \leq j \leq a_i\}$ where $x_{(i,j)}$ is a generators of degree $i$. From ~\cite[Theorem 7 (Witt)]{jacobson2013lie} we have that a basis for the graded components $\mathcal{U}\big(\mathfrak{L}(\vec{a})\big)_n$ is the set of monomials of the form $m=x_{(\alpha_1,k)} \cdots x_{(\alpha_\ell,s)}$ where $\alpha=(\alpha_1,\dots,\alpha_\ell)$ is a set composition of $n$. This gives us that the dimension of each graded components is given by $\sum_ {\alpha \models n} a_{\alpha_1} a_{\alpha_2} ... a_{\alpha_\ell}$. This gives us the results and this conclude the proof.
...
$$\dim(H_n)=\dim(H') \text{ for all } n\geq 0.$$
