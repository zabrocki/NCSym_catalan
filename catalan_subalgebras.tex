...
\title{Free, graded, connected, co-commutative Hopf algebras as integer sequences}
\author{}
...
\begin{abstract}
%attempt 1
We study graded Hopf algebras that are non-commutative and co-commutative
...
if and only if a transformation of the sequence is non-negative.
We give conditions on the sequences of graded dimensions for two Hopf algebras $H$ and $K$
under which there exists a surjective homomorphism from $H$ to $K$.
...
\begin{comment}
%attempt 2
We study the category of free, graded, connected, co-commutative Hopf algebras
by what can be said from the sequence of graded dimensions.
Let $\vec{a} = (a_1, a_2, a_3, \ldots)$ be a sequence of non-negative integers,
then a Hopf algebra $H := \bigoplus_{n\geq0} H_n$ such that 
$a_n = \dim H_n$ if and only if a transformation of $\vec{a}$ is non-negative.
If $H$ and $K$ are graded Hopf algebras then state conditions on the dimension
sequences of $H$ and $K$...
\end{comment}

\section{Introduction}
\mike{rewrite abstract, identify missing references (start by look in the .bib file), proofread!}

This paper is motivated by a question about the Hopf algebra of symmetric functions in noncommutative variables, $\NCSym$~\cite{AgEtAl12, BHRZ05, GebSag01, HNT08, RS06, Wolf36}.  
This is a graded Hopf algebra with dimensions given by the Bell numbers, so bases of $\NCSym$ are naturally indexed by set partitions.  
...
We recognized that some well-known bases of $\NCSym$ had product and coproduct formulas that were closed when restricted to the nonnesting or noncrossing subsets.  
...
We realized that we could resolve Problem~\ref{problem:motivating_calatan_question} by applying a recent result of Aliniaeifard and Thiem~\cite{AT20}.  
This is a consequence of the the category in which $\NCSym$ lies, and applies to a much larger class of Hopf algebras.  
...
In this context,~\cite[Theorem~4.2]{AT20} show that that the isomorphism class of an \FGCCHA $H$ can be fully determined by the \emph{dimension sequence}
\[
\vecdim(H) = \big( \dim(H_{1}), \dim(H_{2}), \dim(H_{3}), \ldots \big)
\qquad\text{where}\qquad
H = \bigoplus_{n \ge 0} H_{n}.
\]
We answer Problem~\ref{problem:motivating_calatan_question} by showing that all Hopf subalgebras of any \FGCCHA (including $\NCSym$) are \FGCCHAs, which follows from the Shirshov--Witt Theorem; see Lemma~\ref{lem:ShiWitt}.  
...
%We realized that all Hopf subalgebras of $\NCSym$ with equal graded dimensions are isomorphic.  
%We also realized that this is a consequence of the category in which $\NCSym$ lies, and applies to a much larger class of Hopf algebras.  
%This category is the full subcategory of free, graded, connected, and cocommutative Hopf algebras, which we will denote by \FGCCHA.  

Properties of the category of \FGCCHAs (or its dual) have been considered in several references~\cite{AT22, Block85, FP24, LodayRonco10, MM65, Reutenauer-FreeLieAlgebras}.  
{\color{red}{Going back to the Milnor--Moore theorem}}, the structure of \FGCCHAs have been characterized by the number of free generators and the dimension of the subspace of primitives.  
...
This paper considers what we can learn about
a single Hopf algebra in the category of \FGCCHAs
from the sequence of the graded dimensions of the 
Hopf algebra.  Building on known results (Theorem~\ref{thm:AT}, Proposition~\ref{prop:LieToFree}, Proposition~\ref{prop:indecomposable generators}, Lemma~\ref{lemma:primitive generators}, and Proposition \ref{lem:ShiWitt}), this perspective allows us determine 
whether maps between \FGCCHAs exist by 
checking properties of the sequences.    
...
if the INVERTi transform of $\vec{h}$.
In the next section, Theorem~\ref{thm:subclassification2} states
...
This classifies all Hopf subalgebras of \FGCCHAs.
...
Therefore $\NCSym$ has a Hopf subalgebra with Catalan dimension, which is unique up to isomorphism.  
In the following example, we concretley identify one representative of this isomorphism class inside of $\NCSym$.  

%%%%%\newpage
%%%%%
%%%%%
%%%%%Hook, introduce $\NCSym$.  The obvious basis of $\NCSym$ is the \emph{monomial basis} whose degree-$n$ elements are
%%%%%\[
%%%%%\sum_{\beta \in S_{n} \cdot \alpha} x_{\beta_{1}} x_{\beta_{2}} \cdots x_{\beta_{n}}
%%%%%\qquad\text{for $\alpha \in \ZZ_{+}^{n}$},
%%%%%\]
%%%%%which can be uniquely indexed by the set partition in which $i$ and $j$ belong to the same part if and only if $\alpha_{i} = \alpha_{j}$.  
%%%%%Thus, the graded dimensions of $\NCSym$ are the Bell numbers $\Bell(n) = \#\{\text{set partitions of $[n]$}\}$.
%%%%%Many other bases of $\NCSym$ can be naturally indexed by set partitions, including those introduced by Rosas and Sagan~\cite{RS06} (and others?).  
%%%%%
%%%%%The Catalan numbers $\Catalan_{n} = \frac{1}{n+1}\binom{2n}{n}$ count several noteworthy classes of set partition, most notably the noncrossing and nonnesting partitions~\cite{Stanley2015}.\lucas{not sure what the best citation for this is - Stanley, OEIS, or something more bespoke?} \mike{I used Stanley's Catalan book as a reference}
%%%%%Our work on the present project began with the following natural question.
%%%%%
%%%%%\begin{problem}
%%%%%\label{problem:Catalan_hopfsub_basis}
%%%%%Given a basis $\mathcal{B} = \{b_{\pi} \;|\; \text{set partitions $\pi$} \}$ of $\NCSym$, do the subsets
%%%%%\[
%%%%%\{b_{\pi} \;|\; \text{noncrossing set partitions $\pi$} \}
%%%%%\qquad\text{or}\qquad
%%%%%\{b_{\pi} \;|\; \text{nonnesting set partitions $\pi$} \}
%%%%%\]
%%%%%span a graded Hopf subalgebra of $\NCSym$?
%%%%%\end{problem}
%%%%%
%%%%%Preliminary computer exploration suggested that Problem~\ref{problem:Catalan_hopfsub_basis} had a positive answer for a relatively large number of bases; see Table~\ref{table:catalan_bases}
%%%%%\mike{if we keep this table, citations for the bases, e.g. $m$, $e$, $h$, $p$ were all from the Rosas/Sagan paper.} for a summary.  
%%%%%In some cases these Hopf subalgebras are obviously isomorphic, but generally the product and/or coproduct differed superficially between bases.  
%%%%%The relative abundance and diversity of these ``Catalan subalgebras'' led us to investigate another natural question.
%%%%%
%%%%%\begin{table}
%%%%%\begin{tabular}{|c|c|c|}
%%%%%\hline
...
%%%%%\hline
%%%%%\end{tabular}
%%%%%\caption{Answers to Problem~\ref{Catalan_hopfsub_basis} for well-known bases of $\NCSym$.}
%%%%%\label{table:catalan_bases}
%%%%%\end{table}
%%%%%
%%%%%\begin{problem}
%%%%%\label{problem:Catalan_hopfsub_classify}
%%%%%Classify all Hopf subalgebras of $\NCSym$ with graded dimension  $\Catalan_{n}$.
%%%%%\end{problem}
%%%%%
%%%%%
%%%%%%\newpage
...
%%%%%
%%%%%In order to answer Problem~\ref{problem:Catalan_hopfsub_classify}, we needed to use structural properties of $\mathsf{NCSym}$.  
...
%%%%%Beyond $\NCSym$, numerous Hopf algebras in combinatorics fall into this category, including each of the ones shown in Table~\ref{}.
%%%%%
%%%%%Our results about FGCCHAs include....\lucas{this is where we explain the framework, including below}
%%%%%
%%%%%
%%%%%\newpage
%%%%%
...
%%%%%H = \bigoplus_{n \ge 0} H_{n}.
%%%%%\]
%%%%%We knew that all Hopf subalgerbas of $\mathsf{NCSym}$ that we found in Table~\ref{} were \FGCCHAs, so all of these are isomorphic.
%%%%%
...
%%%%%
%%%%%These tools enable us to answer one final, much stronger, question.
%%%%%
%%%%%\begin{problem}
%%%%%\label{problem:Catalan_hopfsub_classify_all}
%%%%%Classify all Hopf subalgebras of $\NCSym$ .
...
%%%%%in Section~\ref{}.
%%%%%
%%%%%We give a necessary and sufficient condition for such a copy to exist, answering Problem~\ref{problem:Catalan_hopfsub_classify_all}.  The characterization is in terms of the dimension sequence $\vec{h}$, the graded cardinality of the generating set $\vec{a}$, and the graded dimension of the space of primitives $\vec{p}$.  
%%%%%
%%%%%\begin{center}
%%%%%\begin{tabular}{c|c|c}
...
%%%%%
\begin{example}
We first identify one such Hopf subalgebra explcitly; there are many more examples including the one constructed in~\cite{AT20}.  The power-sum basis elements $\mathbf{p}_{\pi}$ of $\mathsf{NCSym}$ are indexed by set partitions (see~\cite{BHRZ05} for details on the Hopf structure on the power sum basis).  Direct calculation shows that the subset $\{\mathbf{p}_{\pi} \;|\; \text{$\pi$ is noncrossing}\}$ is closed under the product and coproduct, so they span a Hopf subalgebra whose dimension is the Catalan numbers.

In~\cite{NT05} Novelli and Thibon define a graded connected Hopf algebra $\mathbf{CQSym}$ which has a basis $\mathbf{P}^{\pi}$ indexed by non-crossing set partitions $\pi$.  
...
Moreover, the number of non-crossing set partitions is the Catalan numbers $(1, 2, 5, 14, 42, 132, \ldots)$~\cite[\href{https://oeis.org/A000108}{A000108}]{OEIS}, so $\mathbf{CQSym}$ is isomorphic to every Hopf subalgebra of $\NCSym$ whose dimension is also the Catalan numbers.

Using Aliniaefard and Thiem's result (see Theorem~\ref{thm:AT}), $\{\mathbf{p}_{\pi} \;|\; \text{$\pi$ is noncrossing}\}$ and $\mathbf{CQSym}$ are isomorphic.  
However, the product and the coproduct on the bases $\mathbf{P}^{\pi}$ from~\cite{NT05} and $\mathbf{p}_{\pi}$ from~\cite{BHRZ05} are not the same, so this isomorphism is not immediately obtained.  
Theorem~\ref{thm:subclassification2} outlines a procedure (using the Eulerian idempotent) for constructing one of the many isomorphisms that we know must exist.  
\end{example}
