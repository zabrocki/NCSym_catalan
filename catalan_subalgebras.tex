...
The category of \FGCCHAs (or its dual) has been studied generally by~\cite{MM65},~\cite{Block85},~\cite{Reutenauer-FreeLieAlgebras},~\cite{AT22},~\cite{FP24}.  
In addition to $\NCSym$, much work has been put into understanding specific examples including the \FGCCHAs of noncommutative symmetric functions (see Example~\ref{ex:NSym}), permutations (see Example~\ref{example:hopf_algebras_of_permutations}), and trees~\cite{AS05cc, GL09}.


\lucas{roughtideal here}
The category of \FGCCHAs has been studied in the literature before, as the CMM theorem (see Section~\ref{sec:HopfAlgebra}) tells us that it is equivalent to the category of free Lie Algebras.  In particular, \FGCCHAs are characterized equivalently in terms of their free generators and the (dimensions of) the Lie algebra of primitives.  MORE HERE\lucas{``reference, reference, reference''}.
...
\]
We answer Problem~\ref{problem:motivating_calatan_question} by showing that all Hopf subalgebras of $\NCSym$ are \FGCCHAs.  
...
from the sequence of the graded dimensions of the Hopf algebra alone.
