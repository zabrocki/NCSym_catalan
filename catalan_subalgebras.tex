...
graded dimension of the space of primitive elements.  These sequences will be respectively denoted $\{h_n\}_{n \geq 1}$,
$\{ a_n \}_{n \geq 1}$ and $\{ p_n \}_{n \geq 1}$.

It is well known, but we will provide reasoning to support, that the generating functions for these sequences
...
\label{prop:sequences}
    Let $\{ h_n \}_{n \geq 1}$, $\{ a_n \}_{n \geq 1}$ and $\{ p_n \}_{n \geq 1}$ be three seqences of numbers satisfying
    Equation \eqref{eq:gf_relation}, then these sequences are related by the following formulae:
    \begin{itemize}
...
\end{equation}
The equations in item $ii)$ follow by applying M\"obius inversion.
