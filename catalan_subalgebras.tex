...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    Then it is clear that $\phi_{\vec{K}}$ is an isomorphism and $\phi_{\vec{K}}(\vec{G}) = \vec{K} \in \OPG(H)$.
...
\lucas{TODO: cut this example down on detail, add references to the \FGCCHA papers where this appears: Mike says it may appear in two, Bergeron and possibly one other.  If there are good connections, we can keep it after Theorem~\ref{thm:a sequence}}
    ...
    Then $\vec{a}$ corresponds to~\cite[A003319]{OEIS} which counts the number of indecomposable permutations of $[n]$.  By Theorem~\ref{thm:a sequence} there exists a \textsf{FGCCHA} $H$ of graded dimension $n!$.
    
The Hopf algebra $H$ appears in the literature a number of times.  Li~\cite{Li15} 
    
    , which we will construct explicitly.
