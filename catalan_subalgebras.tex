...
\begin{enumerate}
\item what are the bases $b_\pi $ of $\mathrm{NCSym}$ for which $\{ b_\pi :  \text{for $pi$ non-(crossing or nesting)} \}$ is a Hopf subalgebra of dimension Catalan?
...
Checking answers to (1) by computer, we found many bases seemed to give us Hopf subaglebras with the right dimension (transcribe meeting notes from May 13).  However, weren't sure if these were isomorphic or not.
...
The remainder of Question (2a) is therefore asking whether there are Hopf subalgebras of $\mathrm{NCSym}$ which are not \FGCCHAs.  However, we found that this is not true: Theorem~\ref{} states that all Hopf subaglerbas of an \FGCCHA are also \FGCCHAs.  This tells us that there is a unique isomorphism class of Catalan-dimensional Hopf subaglebra of $\mathrm{NCSym}$.

We then considered how we would construct all subalgebras of $\mathrm{NCSym}$ with a given dimension sequence, or more generally for any \FGCCHA.  There are usually infinitely many isomorphic copies when one exists, but we describe a procedure for constructing all isomorphic copies of one \FGCCHA inside another in Section~\ref{}.
...
\end{array}
\]
\lucas{See the paper \url{https://arxiv.org/pdf/2112.08434} for explanation on CMM v.s. MM v.s. CKGMM naming conventions}
...
\[
\vec{G} = \Big(\vec{G}^{(n)} = (g_{1}^{(n)}, g_{2}^{(n)}, \ldots, g_{a_{n}}^{(n)})\Big)_{n = 1}^{\infty}
\qquad\text{with}\qquad
\begin{array}{l}
%\text{(1) $\vec{X}^{(n)} = (x_{1}^{(n)}, x_{2}^{(n)}, \ldots, x_{1}^{(n)})$} \\
\text{(1) each $g_{i}^{(n)} \in \mathcal{P}(H)_{n}$ , and} \\
...
Thus, by Aliniaeifard and Thiem's result (Theorem~\ref{thm:AT})
...
\end{equation}\lucas{TODO: We changed some notation here.  Need to make sure it is reflected throughout the rest of the paper}
...
We will also make use of the \emph{Eulerian idempotent} (see \cite[\S 1.4]{aguiar2005cocommutative} or \cite[\S 4.5.2]{loday2013cyclic})
...
generators for $\mathcal{H}_{HO}$ in~\cite{aguiar2005cocommutative}, as do
Novelli and Thibon for $\mathbf{CQSym}$ in~\cite[\S 5]{NT05}. Lauve and Mastnak use a related primitive idempotent to produce primitive generators for $\mathsf{NCSym}$ in~\cite{LM11}. 
We will use this method holds for any \FGCCHA.
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    Let us fix $\vec{G} = \biguplus_{n \geq 1} \vec{G}^{(n)} \in \OPG(\mathcal{U}(\mathfrak{L}(\vec{a}))$.
    For a graded Hopf algebra isomorphism $\phi:\mathcal{U}(\mathfrak{L}(\vec{a})) \to H$,
    it is clear that $\phi(\vec{G})$ is a set of free homogeneous generators for $H$.
    We may induce an ordering on $\phi(\vec{G})$ from the ordering on $\vec{G}$, so that 
    \[
    \vv{\phi(\vec{G}^{(n)})} = \left(\phi\left(g_1^{(n)}\right), \phi\left(g_2^{(n)}\right), \ldots, \phi\left(g_{a_n}^{(n)}\right)\right),
    \]
    hence $\phi(\vec{G})$ is a uniquely determined member of $\OPG(H)$.

    The assignment $\phi \mapsto \phi(\vec{G})$ is injective,
    because any two \FGCCHA isomorphisms equal on a set of generators extend
    to the same isomorphism. To show surjectivity, consider an arbitrary $\vec{K} \in \OPG(H)$.
    Define $\phi_{\vec{K}}:\mathcal{U}(\mathfrak{L}(\vec{a})) \to H$ by algebraically extending
    the mapping of generators $\phi_{\vec{K}}:g_{i}^{(n)} \mapsto k_i^{(n)}$.
    Then immediately $\phi_{\vec{K}}$ is an isomorphism and $\phi_{\vec{K}}(\vec{G}) = \vec{K} \in \OPG(H)$.

    We now prove the second statement of the theorem.
    \mike{what is the second statement of the theorem precisely?  I see one statement}
    Recall from Definition~\ref{defn:freelie} the free Lie algebra $\mathfrak{L}(X)$ on a graded set $X$.  
    By \cite[Prop. 2.4]{F23},
    for any $G \in \OPG(H)$ we have $\mathcal{P}(H) \cong \mathfrak{L}(G)$.
    By definition, $\mathfrak{L}(G) = \CC G \oplus [\mathfrak{L}(G),\mathfrak{L}(G)]$, therefore
    \[
        \CC G \cong \mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)].
    \]
    \lucas{TODO: define bracket subspace, add citation for this fact}
    Fixing $G \in \OPG(H)$, we can then interpret an arbitrary choice of $S \in \OPG(H)$ as some homogeneous change of basis of the quotient space $\mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)]$ relative to $X$. Each homogeneous component $\vec{Y}^{(n)}$ then has the form
    \[
        \vec{S}^{(n)} = M^{(n)}\vec{G}^{(n)} + \vec{B}^{(n)},
    \]
...
\lucas{Add citations to other $n!$ \FGCCHAs: -
Grossman Larson ``Hopf algebras of heap-ordered trees and permutations'' \cite{GL09},
Teresa X.S. Li ``The monomial basis and the Q-basis of the Hopf algebra of parking functions'' \cite{Li15},
``Commutative combinatorial Hopf algebras'' by Novelli and Thibon (dual) \cite{HNT08},
and the Bergeron one}
\lucas{TODO: cut this example down on detail, add references to the \FGCCHA papers where this appears: Mike says it may appear in two, Bergeron and possibly one other.  If there are good connections, we can keep it after Theorem~\ref{thm:a sequence}}
    ...
    Then $\vec{a}$ corresponds to~\cite[A003319]{OEIS} which counts the number of indecomposable permutations of $[n]$, which we will denote by $\dot{\mathfrak{S}}$.  By Theorem~\ref{thm:a sequence} there exists a \textsf{FGCCHA} $\mathcal{U}(\mathfrak{L}(\dot{\mathfrak{S}}))$ of graded dimension $n!$.
    Viewing each permutation as a word in $\NN$, the product is shifted concatenation of permutations, and the coproduct is defined such that every indecomposable permutation is primitive.
    
    There are numerous \FGCCHA structures on permutations in the literature, such as $k\mathcal{T}$ and $k\mathfrak{S}$~\cite{GL09}, $\mathrm{gr}(\mathfrak{S}Sym)^*$~\cite{aguiar2005cocommutative}, $\mathbf{\mathfrak{S}Sym}$~\cite{HNT08}, and $\mathbb{K}\mathfrak{S}$~\cite{Li15}.
    By Theorem~\ref{thm:AT}, they are all isomorphic to $\mathcal{U}(\mathfrak{L}(\dot{\mathfrak{S}}))$.
    \end{example}
...
\lucas{TODO: This is our OPG Example~\ref{ex:OPGexample}, up to some re-writing and adaptation!}
    ...
    There is a Hopf subalgebra $\mathsf{NCSym}_{\mathrm{NC}}$ of $\mathsf{NCSym}$ spanned by the power-sum basis elements $\mathbf{p}_{\pi}$ indexed also by non-crossing set partitions, and freely generated by $\{\mathbf{p}_{\pi}:\pi \text{ is atomic and non-crossing}\}$ (see ~\cite{BHRZ05} for details on \textsf{NCSym}).
    ...
    \]
    we know $\mathbf{CQSym} \cong \mathsf{NCSym}_{\mathrm{NC}}$. To make the isomorphism explicit, we map $f:\mathbf{e}(\mathbf{P}^{\pi}) \to \mathbf{e}(\mathbf{p}_{\pi})$ for all atomic non-crossing set partitions $\pi$, and extending algebraically gives a Hopf algebra isomorphism $f:\mathbf{CQSym} \to \mathsf{NCSym}_{\mathrm{NC}}$.
\end{example}
