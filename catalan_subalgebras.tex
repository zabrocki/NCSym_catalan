...
In fact, this a consequence that \FGCCHAs are closed under taking graded Hopf subalgebras.  
...
\mike{this is the part that we wrote last time.  I'm brining it to the top so that all the previous
"versions" of the introduction are below.}
...
to describe the structure of these objects.
In Section~\ref{sec:surjection}, Theorem~\ref{thm:surjection} 
shows that there exists a surjective homomorphism from one \FGCCHA 
to another if and only if the INVERTi of the dimension sequences are
comparable.
In the next section, Theorem~\ref{thm:subclassification2} states
that an \FGCCHA occurs as a Hopf subalgebra of
another if and only if the inverse Euler transform of one dimension sequence
dominates the other; this classifies all Hopf subalgebras of \FGCCHAs.
