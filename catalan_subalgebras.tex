...
Theorem~\ref{prop:h sequence} states that an arbitrary nonnegative sequence $\vec{h}$
...
$h_n = a_{n+1} = \frac{1}{n+1}\binom{2n}{n}$, then $p_n$ is equal to the number of
binary Lyndon words of length $2n$ with the same number of $1$s and $0$s.
...
Let $D = d_1d_2\ldots d_{2n}$ be a binary word with an equal number of $1$s and $0$s (we use the notation $|D| = n$).
...
$D_i$ is a (possibly empty) Dyck word and $r+|D_1|+\cdots+|D_r| = n$.
From Example \ref{ex:catalan}, we see that $p_4 = 8$ and these sequences are
$(111000)$, $(110100)$, $(101100)$, $(110010)$, $(101010)$,
$(1100,\bullet)$, $(1010,\bullet)$, $(10, \bullet, \bullet)$.
\mike{I'm not sure this is a great example.  It doesn't use the lex ordering on the alphabet.
Its kind of long.  We don't see the bijection with balanced binary Lyndon words
of length $2n$ (I think I know the bijection, but I wouldn't want to prove it here).
The advantage is that it connects to Example \ref{ex:catalan}.
...
\begin{example}
\lucas{TODO: Insert example}
\end{example}

\subsection{Proof of Theorem~\ref{thm:h sequence}}
...
We now prove Theorem~\ref{thm:h sequence}; recall Proposition~\ref{prop:SequencesAsHopfStructures}.
...
We conclude by giving an alternative sequence-based characterization of \FGCCHAs which follows as a Corollary of Theorem~\ref{thm:h sequence}.\lucas{GROUP TODO: is this worth keeping?  Should we move it to the beginning of the section?  Do we have an example where this is actually a more helpful formulation of the statement?}
\mike{comment about this: there is a bijection between $\{$ sequences $\vec{p} \in \CC^{\NN}$ for which $\phi_{p,a}(\vec{p}) \in \NN^{\NN} \}$ =
and $\{$ sequences $\vec{a} \in \NN^{\NN} \}$.  In fact that bijection is $\phi_{p,a}$.}
...
We now consider the problem of explicitly constructing isomorphisms between \FGCCHAs via the set $\OPG(H)$ defined at the beginning of the section.  We will make use of the \emph{Eulerian Idempotent}
\[
\mathbf{e}(x) = \text{id} (x) - \frac{1}{2}\mu \circ \Delta_{+}(x) + \frac{1}{3} \mu^{(2)} \circ \Delta_{+}^{(2)}(x) - \cdots 
\]
...
\lucas{TODO: Explain how the next result fits into the literature.  I seem to remember it being a novel statement, and that e.g. Aguiar Mahajan actually don't seem to know that it holds in general?}
...
\begin{proof}
        ...
        \mathbf{e}(x) = x + p((x_i)),
        \]
        where $p((x_i))$ is some polynomial in generators $x_i$ of lesser degree than $x$.
        Then the minimal term in $\mathbf{e}(x)$ is $x$, and any polynomial
        $q(\mathbf{e}(x_1),...,\mathbf{e}(x_n))$ has the same minimal term as
        $q(x_1,...,x_n)$. Hence if $f$ is a non-trivial polynomial and
        $f(\mathbf{e}(x_1), \ldots, \mathbf{e}(x_n)) = 0$, then
        $f(x_1, \ldots, x_n) = 0$, a contradiction.
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    We now prove the second statement of the theorem. By \cite[F23]{Prop. 2.4}, for any $X \in \OPG(H)$ we have $\mathcal{P}(H) \cong \mathfrak{L}(X)$. Then, because $\mathfrak{L}(X) = \CC X \oplus [\mathfrak{L}(X),\mathfrak{L}(X)]$ we have
    \[
    ...
    \]
    for some $M^{(n)} \in \GL(a_n,\CC)$ and $\vec{B}^{(n)} = \left(b_1^{(n)},b_2^{(n)},\ldots,b_{a_n}^{(n)}\right)$, for some $b_i^{(n)} \in [\mathcal{P}(H),\mathcal{P}(H)]_n$. We may fix a homogeneous basis $\{v_1,v_2,\ldots,v_{p_n - a_n}\}$ of $[\mathcal{P}(H),\mathcal{P}(H)]_n$, so that $\vec{B}^{(n)}$ can be identified with an $a_n$ by $p_n-a_n$ matrix $K^{(n)}$, such that $b_i^{(n)} = \sum_{j = 1}^{p_n - a_n}K_{ij}^{(n)}v_j$. Then we define the $a_n$ by $p_n$ matrix
    \[
    A^{(n)} = \Bigl[M^{(n)} \big| K^{(n)}\Bigr]
    \]
    as desired. 
    
    Note that in order for $K^{(n)}$ to be defined uniquely there should be no ambiguity in the choice of basis and it's ordering - based on the fixed $X \in \OPG(H)$ we choose the basis of $\mathcal{P}(H)$ indexed by Lyndon words in $X$, ordered lexicographically. Let $V = \biguplus_{n \geq 1} \vec{V}^{(n)}$ be the ordered homogeneous basis of $[\mathcal{P}(H),\mathcal{P}(H)]$ chosen accordingly.
    ...
    \[
    \vec{Z}^{(n)} = P^{(n)}\vec{X}^{(n)}+Q^{(n)}\vec{V}^{(n)}
    \]
    descends to a basis for $\mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)]$. It follows from \cite[F23]{Corollary 2.6} that $Z = \biguplus_{n \geq 1} \vec{Z}^{(n)} \in \OPG(H)$, and it is clear the correspondence is one to one.
\end{proof}
...
\begin{remark}
\lucas{TODO: This is actually a proof of Theorem~\ref{thm:OPGmaps} (ii), should be converted as such.}
    Given two Hopf algebras $H$ and $K$ satisfying the criterion of Prop. 7.2 \felix{what is this criterion?}, we may present a method to compute an explicit isomorphism. Given graded sets $X$ and $Y$ generating $H$ and $K$ respectively, we apply Lemma~\ref{lemma:primitive generators} to obtain primitive generators $\mathbf{e}(X)$ and $\mathbf{e}(Y)$. Choosing bijections $f_n:\mathbf{e}(X)_n \to \mathbf{e}(Y)_n$ for all $n \geq 1$ we obtain a bijection of primitive generators $f:\mathbf{e}(X) \to \mathbf{e}(Y)$ which completely determines an isomorphism $\tilde{f}:H \to K$.
\end{remark}

\subsection{Examples that need a home}
\lucas{GROUP TODO: We need to decide whether to keep these examples.  If we do, they should be edited up to the current notation/terminology and we need to find a home for them.}
...
\begin{example}
	We illustrate that even if two sequences respectively associated to the graded dimension of two \FGCCHAs are comparable, it does not implies that one Hopf algebra is a sub-Hopf algebra of the other. Let us consider the \FGCCHAs $A$ and $B$ with sequences corresponding to the graded numbers of free generators $\vec{a}=(0,1,0,0,\dots)$ and $\vec{b}=(1,0,0,0,\dots)$. Let us denote $\vecdim(A)=\vec{h}$ and $\vecdim(B)=\vec{g} $ and let us see that $\vec{h}\leq \vec{q}$. Following the computation given by
	
	$$h_n = \sum_{\beta \vDash n} a_{\beta_1} a_{\beta_2} \cdots a_{\beta_{\ell(\beta)}}$$
	
	We can see that 
	
	$$\vec{g}_n= 1$$
	
	This comes from the fact that the only set composition $\beta$ that impact the summation is the composition with $n$ parts given by $(1,\dots,1)$. On the other side we have that
	
	$$\vec{h}_n= \left \{ \begin{matrix}
		0 & \textit{ if } n \textit{ is odd} \\
		1 & \textit{ if } n \textit{ is even}
	\end{matrix} \right.$$
	
	This comes from the fact that the only set composition $\beta$ that impact the summation is the even compositions with $n/2$ parts given by $(2,\dots,2)$. This gives us that $\vec{h}$ is domintated by $\vec{g}$. After computation, we have that $(\phi_{a,p}(\vec{a}))_1=1$ and $(\phi_{a,p}(\vec{b}))_1=0$ but we have $(\phi_{a,p}(\vec{a}))_2=0$ and $(\phi_{a,p}(\vec{b}))_2=1$. Hence, those two sequences are not comparable and thus $A$ cannot be a sub-Hopf algebra of $B$.
...
We have already\lucas{GROUP TODO: either add an example of NSym earlier in the text or remove/change this one} identified we have identified $\mathcal{U}(\mathfrak{L}(1, 1, 1, \ldots)) \cong \mathsf{NSym}$, we conclude that $H$ has a Hopf subalgebra isomorphic to $\mathsf{NSym}$.
\end{example}
...
The elements $X$ are linearly independent modulo the derived sub-Lie algebra $[\mathfrak{L}(\vec{a}), \mathfrak{L}(\vec{a})]$, so $a_{n}$ is a lower bound on the dimension of the degree $n$ part of the quotient.
However, any element of homogeneous degree $n$ in $\mathfrak{L}(\vec{a})$ must be,
...
\mike{two assertions are being made in this paragraph that
