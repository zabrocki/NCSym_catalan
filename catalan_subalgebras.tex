...
\begin{example}
	Let us consider the Fibonacci sequence $F$ and the Lucas sequence $L$ as $\vec{h}$ sequence. We can easily check that the $\vec{a}$ sequence coming from $F$ has a Hopf algebra associated to it but as surprisingly as it is, the $\vec{a}$ sequence coming from $L$ does not correspond to any Hopf algebra.
\end{example}
