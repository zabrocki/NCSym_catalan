...
\]
where $\mu$ is the Mobius function, and setting $s(\lambda)$ to be the sum of multiplicities of $\lambda$ and $\lambda^!$ the multipllicity factorial,
\[
W_m(\vec{a}) = \sum_{\lambda \vdash m} \frac{(s(\lambda)-1)!}{\lambda^!}a_{\lambda_1} \cdots a_{\lambda_{\ell}}.
\]
Define 
