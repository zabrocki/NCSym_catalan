...
However, in later sections these sequences come from a combinatorial
Hopf algebra $H$ and have an interpretation when they are non-negative integers.
...
We can use \eqref{eq:gf_relation} to calculate the first few values of $\vec{p} = (1,1,3,8,25, \ldots)$
and it is known \cite{} that $p_n$ is the number of binary Lyndon words of length $2n$
...
\section{Preliminary statements about \FGCCHAs}
...
As in the introduction, we use the abbreviation \FGCCHA to mean \textsf{F}ree \textsf{G}raded \textsf{C}onnected \textsf{C}ocommutative \textsf{H}opf \textsf{A}lgebra.  
By definition, every \FGCCHA $H$ comes with a graded set of generators, 
\[
X = \biguplus_{n \ge 1} X^{(n)} 
\]
for which $X^{(n)} \subseteq H_{n}$ and $X$ freely generates $H$ as an algebra.

The structure of any connected cocommutative Hopf algebra $H$ can be described entirely by its primitive elements. For a Lie algebra $L$, there is a cocommutative Hopf algebra structure on $\mathcal{U}(L)$, with the coproduct given by extending $\Delta(x) = 1 \otimes x + x \otimes 1$ for $x \in L$. The Cartier-Milnor-Moore theorem~\cite[see Theorem 5.18]{MM65} states there is a Hopf algebra isomorphism $\mathcal{U}(\mathcal{P}(H)) \cong H$ induced by the inclusion $\mathcal{P}(H) \to H$. Furthermore, by \cite[Theorem 1.4]{Reutenauer-FreeLieAlgebras} we have $\mathcal{P}(\mathcal{U}(L))=L$.

In the context that $H$ is a \FGCCHA generated by a graded set $X(\vec{a})$, we know $H$ is isomorphic as an algebra to the free algebra on $X(\vec{a})$. It is also a well known result that $\mathcal{U}(\mathfrak{L}(\vec{a}))$ is isomorphic to the free algebra on $X(\vec{a})$~\cite[]{Reutenauer-FreeLieAlgebras}, hence there is an isomorphism of algebras $H \cong \mathcal{U}(\mathfrak{L}(\vec{a}))$. This is not necessarily an isomorphism of Hopf algebras, however we will later see such an isomorphism does exist.
...
\lucas{section intro: 1. outline section, 2. Aliniaeifard--Thiem give us iso if hilbert series are equal. 3. we appraoch this from the sequences perspective. 4. state when Hopf algebra exists based on H-sequence. 5. example 6. state how to construct this iso using OPG's 6. example(?) 7. prove statements in subsections.}
...
In this section we will classify all \FGCCHAs by their graded dimensions. Then, we determine for which sequences there exists a \FGCCHA $H$ with $\vecdim(H)$ and $\vecdim(\mathcal{P}(H))$. 

In \cite[]{AT22}, it is proved that two \FGCCHAs $H$ and $K$ are isomorphic if and only if $\vecdim \left(H\right) = \vecdim \left(K\right)$. As in Proposition \ref{prop:SequencesAsHopfStructures}, $\vecdim(H)$ uniquely determines the triple 
\[ 
(\vecdim(H),\veccard(X),\vecdim(\mathcal{P}(H)))
\] 
...
\section{Classifying SubHopf algebras by sequence}
\lucas{Mike will read + make suggestions}
...
\]
To begin, note that by Proposition~\ref{prop:h sequence}, $\vec{a}$ enumerates
a fixed choice of free homogeneous primitive generators for $\mathfrak{L}(\vec{a})$.
The elements of this set\mike{this set = a fixed choice of free homogeneous generators}
are linearly independent modulo the derived sub-Lie algebra,
so $a_{n}$ is a lower bound on the dimension of the degree $n$ part of the quotient.
...
$[\mathfrak{L}(\vec{a}), \mathfrak{L}(\vec{a})]$ and the degree $n$ primitive
generators, so the dimension of the quotient is at most $a_{n}$.
\mike{two assertions are being made in this paragraph that
I think need to be justified: 1. the elements are lin ind mod $\mathfrak{L}(\vec{a})$
...
and by Theorem/Proposition/Corollary \ref{} conclude
...
\bibliography{bibliography}{}
     ...
     \begin{table}[h]
    \hspace*{-1cm} % Adjust this value to move the table to the left if needed
