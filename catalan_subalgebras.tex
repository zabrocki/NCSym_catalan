...
Eulerian idempotent defined in Equation~\eqref{eq:eulerianidempotent}. 
...
\begin{example}
\lucas{TODO: Swap the roles of a, b and h, k in this example to match the theorem's notation.}
...
\]
Therefore, $\vec{h} \le \vec{k}$, so one might expect that $K$ should contain a Hopf subalgebra isomorphic to $H$.  
...
\begin{example}
\label{ex:NSymInCxy}
\lucas{Add pointer to the other example where $\CC\langle x, y \rangle$ appears}
Recall the \FGCCHA $H = \CC\langle x, y \rangle$ from Example~\ref{}, so that $x$ and $y$ have degree one.  Then $\vec{p} = \vecdim(\mathcal{P}(H))$ is the sequence
...
Since $\phi_{h, a}$ is invertible by Proposition~\ref{}\lucas{TODO: insert reference} $\vecdim(J) = \vecdim(K)$.  We therefore conclude by Theorem~\ref{thm:AT} that $J \cong K$.
