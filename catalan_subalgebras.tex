...
\cite{AT20, BBT14, BHRZ05, B08, F12, LM11, NT05, RS06}
\begin{itemize}
...
A \defn{set partition} $A$ of $[n] := \{1,2, \ldots, n\}$ is a set of sets $A = \{ A_1, A_2, \ldots, A_k \}$ where $A_i \subseteq [n]$,
$A_i \cap A_j = \emptyset$ if $i \neq j$ and $\cup_{i=1}^k A_i = [n]$.  The sets $A_i$ are known as the \defn{parts}
of the set partition.  We indicate that $A$ is a set partition of $n$ by the notation $A \vdash n$ and the \defn{length} of the
