...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    \]
    descends to a basis for $\mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)]$. It follows from \cite[F23]{Corollary 2.6} that $Z = \biguplus_{n \geq 1} \vec{Z}^{(n)} \in \OPG(H)$, and it is clear the correspondence between sequences $\left(A^{(n)}\right)_{n \geq 1}$ and $\OPG(H)$ is one to one.
\end{proof}
