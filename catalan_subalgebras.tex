...
\begin{proposition}
	Given a sequence of nonnegative integers $\mathbf{p} = (p_0, p_1, p_2, \ldots)$. $\mathbf{p}$ correspond to the dimension of the primitives at degree $n$ if and only if 
	
	$$a_n = \sum_{\lambda \vdash n} (-1)^{\ell(\lambda)-1} \prod_{d \geq 1} \binom{p_d}{m_d(\lambda)} \textit{ and } a_n\geq 0.$$ 
