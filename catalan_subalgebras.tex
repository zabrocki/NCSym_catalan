...
Let $\{ c_n \}_{n \geq 1}$ be a sequence of integers indexed by positive integers and $\beta$
a composition of $r > 0$ with length $\ell(\beta)$.  In the formulae below we will use the
notation $c_\beta := c_{\beta_1} c_{\beta_2} \cdots c_{\beta_{\ell(\beta)}}$.
If $\alpha = (\beta_{\sigma(1)}, \beta_{\sigma(2)}, \ldots, \beta_{\sigma(\ell(\beta))})$
for some permutation $\sigma$ of $\{1,2, \ldots, \ell(\beta)\}$ then $c_\alpha = c_\beta$.

\begin{proposition}
\label{prop:sequences}
    The sequences above obey the following relations:
    \begin{itemize}
        \item[i)] $\displaystyle h_n = \sum_{\beta \vDash n} a_\beta = \sum_{\lambda \vdash n} \prod_{d \geq 1} \binom{p_d + m_d(\lambda) -1}{p_d -1}$
        \item[ii)] $\displaystyle p_n = \sum_{d|n} \sum_{\beta \vDash d} \frac{d\cdot \mu(n/d)}{n \cdot \ell(\beta)} a_\beta = \sum_{d|n} \sum_{\beta \vdash d} \frac{d\cdot \mu(n/d) (-1)^{\ell(\beta)-1}}{n \cdot \ell(\beta)} h_\beta$
        \item[iii)] $\displaystyle a_n = \sum_{\beta \vDash n} (-1)^{\ell(\beta)-1} h_\beta = \sum_{\lambda \vdash n} (-1)^{\ell(\lambda)-1} \prod_{d \geq 1} \binom{p_d}{m_d(\lambda)}~$
    \end{itemize}
\end{proposition}
\begin{proof}
...
\begin{proposition}
	...
	$$a_n = \sum_{\beta \vDash n} (-1)^{\ell(\beta)-1} h_\beta \geq 0 \text{ for all } n \geq 0.$$
