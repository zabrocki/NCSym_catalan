...
\begin{itemize} 
    \item $\mathbf{h} = (h_0, h_1, h_2, \ldots)$ where $h_n = \dim H_n$,
    \item $\mathbf{p} = (p_0, p_1, p_2, \ldots)$ where $p_n = \dim P(H)_n$,
    \item $\mathbf{a} = (a_0 = 0, a_1, a_2, \ldots)$ where $a_n$ is the number of generators in degree $n$.
\end{itemize}

\begin{proposition}
    The sequences above obey the following relations:
    \begin{itemize}
        \item[i)] $\displaystyle h_n = \sum_{\beta \vDash n} a_{\beta_1} a_{\beta_2} \cdots a_{\beta_{\ell(\beta)}} = \sum_{\lambda \vdash n} \prod_{d \geq 1} \binom{p_d + m_d(\lambda) -1}{p_d -1}$
        \item[ii)] $\displaystyle p_n = \sum_{d|n} \sum_{\beta \vDash d} \frac{d\cdot \mu(n/d)}{n \cdot \ell(\beta)} a_{\beta_1} a_{\beta_2} \cdots a_{\beta_{\ell(\beta)}} = \sum_{d|n} \sum_{\beta \vdash d} \frac{d\cdot \mu(n/d) (-1)^{\ell(\beta)-1}}{n \cdot \ell(\beta)} h_{\beta_1} h_{\beta_2} \cdots h_{\beta_{\ell(\beta)}}$
        \item[iii)] $\displaystyle a_n = \sum_{\beta \vDash n} (-1)^{\ell(\beta)-1} h_{\beta_1} h_{\beta_2} \cdots h_{\beta_{\ell(\beta)}} = \sum_{\lambda \vdash n} (-1)^{\ell(\lambda)-1} \prod_{d \geq 1} \binom{p_d}{m_d(\lambda)}~$
    \end{itemize}
\end{proposition}
