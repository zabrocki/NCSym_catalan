...
This is reminiscent of Foissy's work on cofree Hopf algebras~\cite{}, but we are still surprised by the definitive nature of our results on \FGCCHAs and expect them to be useful tools in future work.  
\begin{enumerate}[itemsep = 1ex]
\item We give an elementary description of all isomorphism classes of \FGCCHA: Theorem~\ref{} states that an arbitrary nonnegative sequence $\vec{h}$ is the dimension sequence for an isomorphism class of \FGCCHAs if and only if it remains nonnegative under a simple arithmetic transformation $\phi_{h, a}$ that we define in Section~\ref{}.

\item W show that every sub-Hopf algebra of an \FGCCHA is itself an \FGCCHA and classify these using a second transformation of the dimension sequence that we  denote by $\phi_{h, p}$.  Theorem~\ref{} states that there is an injective homomorphisms $H \to K$ between \FGCCHAs $H$ and $K$ if and only if $\phi_{h, p}(\vecdim(H))$ is dominated by $\phi_{h, p}(\vecdim(K))$ term-wise, while Theorem~\ref{} give an algorithm to construct all such maps.  
...
We can use Proposition~\ref{prop:sequences} to calculate the first few values of $\vec{p} = (1,1,3,8,25, \ldots)$ and it is known \cite{}\lucas{need proof/cite} that $p_n$ is the number of binary Lyndon words of length $2n$
...
\lucas{Add details to this section about universal enveloping algebra, CMM, PBW theorems.  Include citations to Reutenauer's text.  Pay attention to what we need in ``classification by sequences'' section.}
...
By the Poincare--Birkhoff--Witt theorem, for any fixed homogeneous basis $Y = \biguplus_{n \ge 0} Y^{(n)}$ of $\mathcal{P}(H)$, $h_{n}$ counts the multisets of $Y$ whose elements have degree summing (with repetition) to $n$.  \lucas{Could also use Lyndon factorization if we added it to the previous Proposition} 
Since $\vec{p} = \veccard{Y}$, $h_{n}$ is therefore the coefficient of $t^{n}$ in
\[
\prod_{d \geq 1} \frac{1}{(1-t^d)^{p_d}},
\]\lucas{This should just be the multiset formula.  Not chaning to avoid conflicts.}
so it follows that $\vec{p} = \phi_{h, p}(\vec{p})$.
\end{proof}
...
\begin{example}
\lucas{Insert example}
\end{example}

\subsection{Proof of Theorem~\ref{thm:h sequence}}

We will make use the following established resultsabout free Lie algebras from~\cite{Reutenauer-FreeLieAlgebras}.
...
\begin{proof}
\lucas{haven't edited this proof}
...
\lucas{re-written up to here, less examples and filling the sections above.}

\subsection{Old things}
\begin{proposition}
    Let $H$ be a \FGCCHA generated by a graded set $X(\vec{a})$. Then,
    \begin{enumerate}
    \item $H \cong \mathcal{U}(\mathfrak{L}(\vec{a}))$ as Hopf algebras.
    \item $\mathcal{P}(H) \cong \mathfrak{L}(\vec{a})$.
    \end{enumerate} 
\end{proposition}

\begin{proof}
\begin{enumerate}
\item Because $\vecdim(H) = \vecdim(\mathcal{U}(\mathfrak{L}(\vec{a})))$.
\item Given (1), we have $\mathcal{P}(H) \cong \mathcal{P}(\mathcal{U}(\mathfrak{L}(\vec{a}))) = \mathfrak{L}(\vec{a})$.
\end{enumerate}
\end{proof}

\eric{Prop 6.1 and 6.2 can be moved here}
...
\lucas{Everything in this section should be moved to the previous one, I think.  Leave the section here for numbering reasons until next meeting though.}


In this section we will characterize free graded connected cocommutative Hopf algebras in terms of sequences. 


\begin{proposition}
\label{prop:h sequence}
	Given a sequence of nonnegative integers $\vec{h} = (h_0, h_1, h_2, \ldots)$, there exists a \FGCCHA $H$ with $\vecdim(H) = \vec{h}$ if and only if $\phi_{h,a}(\vec{h}) \geq 0$.
...
\begin{example}
	...
	Having in hand the above proposition, we illustrate that even if two sequences respectively associated to the graded dimension of two \FGCCHAs are comparable, it does not implies that one Hopf algebra is a sub-Hopf algebra of the other. Let us consider the \FGCCHAs $A$ and $B$ with sequences corresponding to the graded numbers of free generators $\vec{a}=(0,1,0,0,\dots)$ and $\vec{b}=(1,0,0,0,\dots)$. Let us denote $\vecdim(A)=\vec{h}$ and $\vecdim(B)=\vec{q} $ and let us see that $\vec{h}\leq \vec{q}$. Following the computation given by
...
sequence $((A_n,\vec{B}_n))_{n=1}^{\infty}$ with
$A_n \in \GL_{a_n}(\mathbb{C})$ and $\vec{B}_n = (b_n^1,b_n^2, \ldots ,b_n^{a_n})$
...
\lucas{Is it possible to find an example of this construction that is small enough to me manageable but big enough to be interesting here?  Maybe with 3 generators?}
...
\begin{example}
\lucas{Examples from earlier versions....do we keep?}
The Lie algebra of primitives $\mathcal{P}(H)$ is the free Lie algebra $\mathfrak{L}_{\{x, y, z, w\}}$.  Now consider the Lie subalgebra
\[
M = \CC\operatorname{-span}\{[x, y]\} \subseteq \mathfrak{L}_{\{x, y, z, w\}},
\]
