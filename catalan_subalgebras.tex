...
Moreover, the number of non-crossing set partitions is the Catalan numbers $(1, 2, 5, 14, 42, 132, \ldots)$~\cite[\href{https://oeis.org/A000108}{A000108}]{OEIS}, so $\mathbf{CQSym}$ is isomorphic to every Hopf subalgebra of $\NCSym$ with Catalan dimension sequence. 

In order to construct an explicit isomorphism, we first identify one such Hopf subalgebra.  The power-sum basis elements $\mathbf{p}_{\pi}$ of $\mathsf{NCSym}$ are indexed by set partitions (see ~\cite{BHRZ05} for details on the Hopf structure on the power sum basis).  Direct calculation shows that the non-crossing subset of power sums is closed under the product and coproduct, so we have a Hopf subalgebra whose dimension is also the Catalan numbers.
