...
The paper is structured as follows.  Sections~\ref{sec:SequencePrelims} and~\ref{sec:HopfAlgebra} state the relationship between the dimension of an \FGCCHA, the number of free generators, and the dimension of the Lie algebra of primitives, with Section~\ref{sec:SequencePrelims} taking on the combinatorial matter and Section~\ref{sec:HopfAlgebra} focusing on the algebraic content.  
Section~\ref{sec:isoclassification} 
Section~\ref{} 
...
\begin{remark}
Using the sequence transfer maps from Definition~\ref{defn:sequencetransfer}, Theorem~\ref{thm:subclassification2} imlies that isomorphisms of Hopf subalgebras of $H$ are also indexed by sequences $\vec{q} \in \NN^{\ZZ_{+}}$ for which $\vec{p} \ge \vec{q}$ and $\phi_{p, a}(\vec{q}) \in \NN^{\ZZ_{+}}$.
\end{remark}
