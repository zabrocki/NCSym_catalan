...
\]
Finally, the cyclic shifts of $w' = w_{1}w_{2}\cdots w_{k}$ are distinct, so taking the unique minimal one, we obtain a Lyndon word of degree $d$.  Since $w$ is the $\ell/k$-fold concatenation of $w'$ with itself, this is uniquely determined by the $d$-elements of the $\mathsf{fcyc}$-orbit of $(0, w)$.
\end{proof}
