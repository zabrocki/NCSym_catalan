...
This paper is motivated by a question about the Hopf algebra of symmetric functions in noncommutative variables, $\NCSym$\lucas{``cite multiple papers''}.  
...
In this context,~\cite[Theorem~?.?]{AT20} show that that the isomorphism class of an \FGCCHA $H$ can be fully determined by the \emph{dimension sequence}
\[
...
%\lucas{roughtideal here}
%The category of \FGCCHAs has been studied in the literature before, as the CMM theorem (see Section~\ref{sec:HopfAlgebra}) tells us that it is equivalent to the category of free Lie Algebras.  MORE HERE\lucas{``reference, reference, reference''}.
%
%The category of \FGCCHAs has been studied in the literature before:
%generally \cite{MM65}, \cite{Block85}, \cite{AT22}, \cite{FP24}, \cite{HNT04}, \cite{Reutenauer-FreeLieAlgebras}
%specfically: NSym, NCSym, \cite{HNT08},

%In particular, we know that up to isomorphism, there is exactly one Hopf subagebra of $\NCSym$ of dimension Catalan (which is isomorphic to the Catalan Quasiymmetric Hopf subalgebra $\mathsf{PQSym}$~\cite{}).  
