...
\lucas{Add citations to other $n!$ \FGCCHAs: - Grossman Larson ``Hopf algebras of heap-ordered trees and permutations'', Teresa X.S. Li ``The monomial basis and the Q-basis of the Hopf algebra of parking functions'', ``Commutative combinatorial Hopf algebras'' by Novelli and Thibon (dual), and the Bergeron one}
\lucas{TODO: cut this example down on detail, add references to the \FGCCHA papers where this appears: Mike says it may appear in two, Bergeron and possibly one other.  If there are good connections, we can keep it after Theorem~\ref{thm:a sequence}}
    ...
    The Hopf algebra $H$ appears in the literature a number of times.  Li~\cite{arxiv:1504} 
