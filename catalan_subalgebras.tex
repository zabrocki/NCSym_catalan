...
The Poincar\'{e}--Brikhoff--Witt Theorem~\cite[Section 0.1]{Reutenauer-FreeLieAlgebras}
...
What remains is to determine which sequences $\vec{h} \in \QQ^{\ZZ_+}$
appear as the graded dimension of an \FGCCHA, a novel result.  In fact,
...
For a graded set $X = \biguplus_{n\ge 1} X^{(n)}$, the free graded Lie algebra $\mathfrak{L}(X)$ is the smallest subspace of $\mathsf{T}(\CC X)$ that contains $X$ and is closed under the commutator bracket, $[x,y] = x \otimes y - y \otimes x$.\lucas{TODO: Move to section 4, near the definition of $\mathsf{T}(\CC X)$.  Needs to be clear why this is graded.}
...
We now state our classification; recall the map $\phi_{h, a}$ from
...
\end{array}.
\]
In particular, $\vec{h} \in \QQ^{\ZZ_+}$ is the dimension sequence of an \FGCCHA if and only if $\phi_{h, a}(\vec{h}) \in \NN^{\ZZ_+}$.  \lucas{added given the words abot the theorem.  OK?}
\end{thm}
...
This Hopf algebra is sometimes referred to as the peak algebra~\cite[Section 2]{Bergeron_2002}.
...
Given a sequence $\vec{a} \in \NN^{\ZZ_+}$, Proposition~\ref{prop:LieToFree} states that $\mathcal{U}(\mathfrak{L}(\vec{a}))$ is an \FGCCHA freely generated by a set with graded cardinality $\vec{a}$, so the given map is well-defined.  Now consider injectivity.  By Proposition~\ref{prop:SequencesAsHopfStructures},
\[
...
Finally for surjectivity, fix an isomorphism class of \FGCCHA, let $H$ be a representative of that class, and let $\vec{h} = \vecdim(H)$.  Then the sequence $\vec{a} = \phi_{h, a}(\vec{h})$ belongs to the domain, since by Proposition~\ref{prop:SequencesAsHopfStructures} $\vec{a}$ is the graded cardinality of its free generating set.  
...
\[
\begin{array}{rcl}
\left\{ \begin{array}{c} \text{sequences $\vec{p} \in \QQ^{\ZZ_+}$ for which $\phi_{p,a}(\vec{p})$} \\ \text{contains only nonnegative integers} \end{array} \right\} & \to & \left\{ \begin{array}{c} \text{Isomorphism classes} \\ \text{of \FGCCHA} \end{array} \right\} \\[2ex]
...
\[
H^{+} = \bigoplus_{n \geq 1} H_n \text{ and } H^{+2} = \bigoplus_{n \geq 1} H_n^{+2},
\]
where,
\begin{equation}
\label{eq:Hplusdef}
H_{n}^{+2} = \bigoplus_{\substack{\alpha \vDash n \\ \alpha \neq (n)}} H_{\alpha_{1}} H_{\alpha_{2}}\cdots H_{\alpha_{\ell}} \subseteq H_{n}.
\end{equation}
By definition (see the beginning of Section~\ref{sec:HopfAlgebra}), the degree $n$ generators of $H$ descend to a basis of $H_{n}/H_{n}^{+2}$, and therefore for any graded generating set $X = \biguplus_{n \ge 1} X^{(n)}$ of $H$, 
\[
\dim(H_{n}/H_{n}^{+2}) = |X^{(n)}|.
\]
There is a further relation between the quotient $H^+/H^{+2}$ and generators of $H$, proven in \cite{F23}.

\begin{proposition}[{\cite[Proposition 2.2]{F23}}]
\label{prop:indecomposable generators}
If $A$ is a free graded connected algebra, and $X \subseteq A$ descends to a basis for $A^{+}/A^{+2}$, then $X$ freely generates $A$.
\end{proposition}

We will also make use of the \emph{Eulerian Idempotent} (see 
%\cite[Section 1.4]{AL15},\cite{aguiar2005cocommutative} ,
\cite[\S 3]{GERSTENHABER1991263},\cite[\S 4.5.2]{loday2013cyclic},\cite[\S 8.4]{Reutenauer-FreeLieAlgebras})\lucas{TODO: Add specific sections to each source that is missing one.} 
\[
...
Aguiar and Sottile use the Eulerian Idempotent to produce primitive generators in~\cite[Theorem 3.4]{aguiar2005cocommutative}, as do Novelli and Thibon in~\cite[Section 5]{NT05}. We will prove this method holds for any \FGCCHA.
\lucas{TODO: Explain how the next result fits into the literature.  Lauve--Mastanak make a specific version of this statement in one of their papers, which is why us giving a general statement and proof is worth doing.}
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    \lucas{TODO: add reference to wherever $\mathfrak{L}(X)$ is defined, add citation for this fact}we have
    \[
...
\lucas{TODO: cut this example down on detail, add references to the \FGCCHA papers where this appears: Mike says it may appear in two, Bergeron and possibly one other.  If there are good connections, we can keep it after Theorem~\ref{thm:a sequence}}
    ...
    Then $\vec{a}$ corresponds to~\cite[A003319]{OEIS} which counts the number of indecomposable permutations of $[n]$. By \ref{thm: Hopf dimension isomorphic} there exists a \textsf{FGCCHA} of graded dimension $n!$, which we will construct explicitly.
...
\lucas{TODO: This is our OPG Example~\ref{ex:OPGexample}, up to some re-writing and adaptation!}
    The \FGCCHA $\mathbf{CQSym}$ introduced in ~\cite{NT05} has a basis $\mathbf{P}^{\pi}$ indexed by non-crossing set partitions $\pi$, and is freely generated by $\{\mathbf{P}^{\pi}:\pi \text{ is atomic and non-crossing}\}$~\cite[Proposition 5.2]{NT05}.

    There is a sub-Hopf algebra $\mathsf{NCSym}_{\mathrm{NC}}$ of $\mathsf{NCSym}$ spanned by the power-sum basis elements $\mathbf{p}_{\pi}$ indexed also by non-crossing set partitions, and freely generated by $\{\mathbf{p}_{\pi}:\pi \text{ is atomic and non-crossing}\}$ (see ~\cite{BHRZ05} for details on \textsf{NCSym}).
    
    The number of non-crossing set partitions of $[n]$ are enumerated by the Catalan numbers, $(C_n)_{n \geq 1}$. Given that 
    \[
    \vecdim(\mathbf{CQSym}) = \vecdim(\mathsf{NCSym}) = (C_n)_{n \geq 1},
    \]
    we know that abstractly $\mathbf{CQSym} \cong \mathsf{NCSym}_{\mathrm{NC}}$.
    ...
    \]
    induces a Hopf algebra isomorphism $\tilde{f}:\mathbf{CQSym} \to \mathsf{NCSym}_{\mathrm{NC}}$.
\end{example}
...
Let $X \in \OPG(H)$ and $Y \in \OPG(K)$ be ordered primitive generating sets as in Definition~\ref{def:OPG}, and let $\vec{a} = \veccard(X)$ and $\vec{b} = \veccard(Y)$. 
...
\item For every surjective homomorphism $f: H \to K$, there exists \lucas{add: non-unique} ordered primitive generating sets $X \in \OPG(H)$ and $Y \in \OPG(K)$ for which $f = f^{X}_{Y}$ as in Equation~\eqref{eq:defaultsurjection}.
...
\]
As $f$ is an graded surjective algebra homomorphism, it must map $H_{n}$ onto $K_{n}$ and $H_{n}^{+2}$ onto $K_{n}^{+2}$, and therefore induce a surjective linear transformation from $H_{n} / H_{n}^{+2}$ to $K_{n} / K_{n}^{+2}$.  Dimension considerations now give that $a_{n} \ge b_{n}$, so the ``only if'' part of (1) holds.  

We continue with our fixed $n$ for (2).  Using standard properties of linear maps, we choose an ordered subset $X^{(n)} = \big( x^{(n)}_{1}, x^{(n)}_{2}, \ldots, x^{(n)}_{a_{n}} \big)$ of $H_{n}$ with two properties: first, $f( x^{(n)}_{i}) \in K_{n}^{+2}$ for all $i > b_{n}$, and second, the set
\[
Y^{(n)} = \big(f(x^{(n)}_{1}), f(x^{(n)}_{2}), \ldots, f(x^{(n)}_{b_{n}}) \big)
\]
descends to a basis of $K_{n} / K_{n}^{+2}$.  
By possibly replacing each $x^{(n)}_{i}$ with its image $\mathbf{e}(x^{(n)}_{i})$ under the Eulerian idempotent, which preserves $H_{n}/H^{+2}_{n}$, we can assume that the elements of $X^{(n)}$ and $Y^{(n)}$ are primitive without loss of generality.  
Further, by possibly adding primitive elements of $H_{n}^{+2}$ to each $x^{(n)}_{i}$, which does not change the defining properties above, we can assume that 
\[
f(x^{(n)}_{i}) = 0 \qquad \text{for all $i > b_{n}$}.
\]

Now, as they span $H_{n}/H_{n}^{+2}$ and $K_{n}/K^{+2}_{n}$, we have produced a set of primitive degree $n$ generators for $H$ and $K$ for all $n \ge 1$.  
Thus, with
\[
