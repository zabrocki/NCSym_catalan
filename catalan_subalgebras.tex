...
Let $\CC^{\NN}$ denote the space of infinite sequences $(c_{n})_{n \ge 1} = (c_{1}, c_{2}, \ldots)$ of complex numbers, i.e.~$c_{n} \in \CC$ for all $n \ge 1$.\mike{TODO: also do $\NN^\NN$, resolve whether we even need to
refer to complex sequences and instead limit ourselves to $\mathbb{Z}$, $\NN$ and potentially $\mathbb{Q}$.}
...
\end{enumerate}
\end{proposition}
\mike{TODO: if h sequence is integers then the p sequence is integers.  Is this obvious?
Is it true?
should we remark that it is the case?}

\felix{If h sequence is integers how can p not be an integer. From (i) wouldn't that implies that $h_n$ is not well define?}
...
\end{equation}
with multiplication given by $\otimes$ and multiplicative grading so that all elements of $\CC X^{(k)}$ have degree $k$.  
...
of compositions of $n$ that are Lyndon \cite[Section 4]{H05}.\mike{check if maybe the correct reference is https://arxiv.org/pdf/math/0410365} \felix{Both are stating this result. Both are showing that it $P_\alpha$ where $\alpha\in LYN$ is a basis of $\mathcal{P}(\mathsf{NSym})$} \felix{page 5 in the article proposed by Mike}
\end{example}
...
\end{array}.
\]
In particular, $\vec{h} \in \CC^{\NN}$ is the dimension sequence of an \FGCCHA if and only if $\phi_{h, a}(\vec{h}) \in \NN^{\NN}$.  \lucas{added given the words abot the theorem.  OK?}
\end{thm}
...
This Hopf algebra is sometimes referred to as the peak algebra(see \cite[Section 2]{Bergeron_2002}).
\mike{TODO: citation?}
...
\begin{example}
\label{ex:OPGexample}
\lucas{TODO: Clean this example up.  Or delete?}
...
Thus, in the notation of Theorem~\ref{thm:OPGmaps}, we have $\vec{a} = (1, 2, 0, \ldots)$ and $\vec{p} = \phi_{a, p}(\vec{a}) = (0, 1, \ldots)$. 
...
Given a sequence $\vec{a} \in \NN^{\NN}$, Proposition~\ref{prop:LieToFree} states that $\mathcal{U}(\mathfrak{L}(\vec{a}))$ is an \FGCCHA freely generated by a set with graded cardinality $\vec{a}$, so the given map is well-defined.  Now consider injectivity.  By Proposition~\ref{prop:SequencesAsHopfStructures},
\[
\vecdim\big(\mathcal{U}(\mathfrak{L}(\vec{a}))\big) = \phi_{a, h}\big(\vec{a}),
\]
which by Aliniaeifard and Thiem's result (Theorem~\ref{thm:AT}) is an invariant of the isomorphism type.  Since the map $\phi_{a, h}$ is injective by~\ref{}\lucas{TODO: add number for where we say the maps are injective}, for any two sequences $\vec{a}, \vec{b}$ in the domain, 
\[
...
Finally for surjectivity, fix an isomophism class of \FGCCHA, let $H$ be a representative of that class, and let $\vec{h} = \vecdim(H)$.  Then the sequence $\vec{a} = \phi_{h, a}(\vec{h})$ belongs to the domain, since by Proposition~\ref{prop:SequencesAsHopfStructures} $\vec{a}$ is the graded cardinality of its free generating set.  
Moreover, we claim that our isomorphism class is the image of $\vec{a}$.  
As we deduced in the previous paragraph, the dimension sequence of $\mathcal{U}(\mathfrak{L}(\vec{a}))$ is $\vec{h}$, which by definition is the dimension sequence of $H$.  
Thus, by Aliniaeifard and Theim (Theorem~\ref{thm:AT}), $H \cong \mathcal{U}(\mathfrak{L}(\phi_{h, a}(\vec{h})))$
\end{proof}

We conclude by giving an alternative sequence-based characterization of \FGCCHAs which follows as a Corollary of Theorem~\ref{thm:a sequence}.\lucas{TODO: fold this into a remark after theorem 2.}
...
We now consider the problem of explicitly constructing isomorphisms between \FGCCHAs via the set $\OPG(H)$ defined at the beginning of the section.  Our proofs in this section and the next will make use of the subspace
\begin{equation}
\label{eq:Hplusdef}
H_{n}^{+} = \bigoplus_{\substack{\alpha \vDash n \\ \alpha \neq (n)}} H_{\alpha_{1}} H_{\alpha_{2}}\cdots H_{\alpha_{\ell}} \subseteq H_{n}.
\end{equation}
By definition (see the beginning of Section~\ref{sec:HopfAlgebra}), the degree $n$ generators of $H$ descend to a basis of $H_{n}/H_{n}^{+}$, and therefore for any graded generating set $X = \biguplus_{n \ge 1} X^{(n)}$ of $H$, 
\[
\dim(H_{n}/H_{n}^{+}) = |X^{(n)}|.
\]

We will also make use of the \emph{Eulerian Idempotent}\lucas{TODO: Add original citation or reference (text) for eulerian idempotent.  First place to look: Aguiar--Lauve's ``Adams operator'' paper.  N.B.: Mike had identified a citation for this in the google chat.}
\[
...
Let $X \in \OPG(H)$ and $K \in \OPG(K)$ be ordered primitive generating sets as in Definition~\ref{def:OPG}, and let $\vec{a} = \veccard(X)$ and $\vec{b} = \veccard(Y)$.  
If $\vec{a} \ge \vec{b}$, projecting from $X$ to $Y$ gives a surjective homomorphism:
\lucas{Group TODO: how do we feel about this notation}
\begin{equation}
\label{eq:defaultsurjection}
\begin{array}{rcl}
f^{X}_{Y}: H & \to & K \\[1ex]
x^{(n)}_{i} & \mapsto & \begin{cases}
y^{(n)}_{i} & \text{for $1 \le i \le b_{n}$} \\
0 & \text{otherwise}
\end{cases}
\end{array}
\end{equation}
where $x^{(n)}_{i}$ and $y^{(n)}_{i}$ denote the $i$th element of the degree $n$ parts of $X$ and $Y$, respectively.  
...
\item For every surjective homomorphism $f: H \to K$, there exists ordered primitive generating sets $X \in \OPG(H)$ and $Y \in \OPG(K)$ for which $f = f^{X}_{Y}$ as in Equation~\eqref{eq:defaultsurjection}.
...
\begin{example}
Recall the \FGCCHA $H = \CC\langle x, y, z \rangle$ from Example~\ref{ex:OPG}, and let $K = \CC\langle u, v \rangle$ denote the \FGCCHA freely generated by two degree-one primitive elements $u$ and $v$.  
...
\[
X = \Big(\big( x, y\big), \big( z - xy-yz \big), \emptyset, \ldots \Big)
\qquad
\text{and}
\qquad
...
f(z - xy-yz) = 0,
\]
so $f = f^{X}_{Y}$ as predicted by the theorem.
\end{example}
...
Recall the subspaces $H_{n}^{+} \subseteq H_{n}$ and $K^{+}_{n} \subseteq K_{n}$ defined in Equation~\eqref{eq:Hplusdef}, and that 
\[
\dim(H_{n} / H_{n}^{+}) = a_{n}
\qquad\text{and}\qquad
\dim(K_{n} / K_{n}^{+}) = b_{n}.
\]
As $f$ is an graded surjective algebra homomorphism, it must map $H_{n}$ onto $K_{n}$ and $H_{n}^{+}$ onto $K_{n}^{+}$, and therefore induce a surjective linear transformation from $H_{n} / H_{n}^{+}$ to $K_{n} / K_{n}^{+}$.  Dimension considerations now give that $a_{n} \ge b_{n}$, so the ``only if'' part of (1) holds.  

We continue with our fixed $n$ for (2).  Using standard properties of linear maps, we choose an ordered subset $X^{(n)} = \big( x^{(n)}_{1}, x^{(n)}_{2}, \ldots, x^{(n)}_{a_{n}} \big)$ of $H_{n}$ with two properties: first, $f( x^{(n)}_{i}) \in K_{n}^{+}$ for all $i > b_{n}$, and second, the set
\[
Y^{(n)} = \big(f(x^{(n)}_{1}), f(x^{(n)}_{2}), \ldots, f(x^{(n)}_{b_{n}}) \big)
\]
descends to a basis of $K_{n} / K_{n}^{+}$.  
By possibly replacing each $x^{(n)}_{i}$ with its image $\mathbf{e}(x^{(n)}_{i})$ under the Eulerian idempotent, which preserves $H_{n}/H^{+}_{n}$, we can assume that the elements of $X^{(n)}$ and $Y^{(n)}$ are primitive without loss of generality.  
Further, by possibly adding primitive elements of $H_{n}^{+}$ to each $x^{(n)}_{i}$, which does not change the defining properties above, we can assume that 
\[
f(x^{(n)}_{i}) = 0 \qquad \text{for all $i > b_{n}$}.
\]

Now, as they span $H_{n}/H_{n}^{+}$ and $K_{n}/K^{+}_{n}$, we have produces a set of primitive degree $n$ generators for $H$ and $K$ for all $n \ge 1$.  
Thus, with
\[
X = \biguplus_{n \ge 1} X^{(n)}
\qquad\text{and}\qquad
Y = \biguplus_{n \ge 1} Y^{(n)}
\]
we have $X \in \OPG(H)$, $Y \in \OPG(K)$, and $f = f^{X}_{Y}$.
\lucas{Something is still slightly off about the logic or writing on this proof... suggestions welcome}
\end{proof}
...
We now state and prove an intermediate result in the proof of Theorem~\ref{thm:subclassification2}; the proof of the theorem follows.  
