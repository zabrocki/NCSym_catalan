...
\end{definition}
\lucas{Should just define as tensor algebra?}
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    We now prove the second statement of the theorem. By \cite[F23]{Prop. 2.4}, for any $X \in \OPG(H)$ we have $\mathcal{P}(H) \cong \mathfrak{L}(X)$.\lucas{Have we defined $\mathfrak{L}(X)$?} Then, because $\mathfrak{L}(X) = \CC X \oplus [\mathfrak{L}(X),\mathfrak{L}(X)]$\lucas{Do we need a cite for this?} we have
    \[
...
\]
Therefore, $\vec{h} \le \vec{k}$, so one might expect that $K$ should contain a subHopf algebra isomorphic to $H$.  However, Theorem~\ref{thm:subclassification2} tells us this is not the case, as $\vec{a}$ and $\vec{b}$ are incomparable.\lucas{I changed the letters for the ridiculous reason that reading it with $A$ and $B$ somehow felt less surprising.... of course $A$ and $B$ depend on $\vec{a}$ and $\vec{b}$ rather than $\vec{h}$ and $\vec{k}$!  But $H$ and $K$? Those should depend on $\vec{h}$ and $\vec{k}$!}
\end{example}
...
\lucas{I think this is resolved}
\end{proof}
...
\]
so that $\vecdim(J) = \vecdim(K)$.  We therefore conclude by Theorem~\ref{thm:h sequence} that $J \cong K$.  
