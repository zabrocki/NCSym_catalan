...
\begin{proposition}
	...
	Then the \(\vec{p}\) and \(\vec{a}\) determine a Hopf algebra. \lucas{Change to constructive statement like in 6.2}
\end{proposition}

\begin{proof}
\lucas{Cross-reference with Proposition~\ref{prop:sequences}.  Nicolas will do.}
	
    Let \(H\) be a free graded connected cocommutative Hopf algebra and let \(\vec{p} = (p_0, p_1, p_2, \ldots)\) 
    be a nonnegative integer sequence where \(\dim P(H)_n = p_n\).
    ...
    cocommutative Hopf algebra \(H\), and we let \(p_n\) count the number of primitive elements of \(H\) at degree \(n\). Hence \(\dim P(H)_n = p_n \geq 0\).
