...
\CC\operatorname{-span}\{x_{i_{1}} x_{i_{2}} \cdots x_{i_{\ell}} \;|\; x_{i_{k}} \in X_{\alpha_{k}}\},
\]
where the degree zero component is $\CC 1_{\mathcal{F}(X)}$.  This algebra $\mathcal{F}(X)$ has the following \emph{universal property}:

\begin{property}
Let $A = \bigoplus_{n \ge 0} A_{i}$ be a graded algebra.  For every function $\phi: X \to A$ such that $\psi(X_{i}) \subseteq A_{i}$, there exists a unique graded algebra homomorphism $\tilde{\phi}: \mathcal{F}(X) \to A$ such that $\tilde{\phi}|_{X} = \phi$, i.e.
\[
\begin{tikzpicture}
\node at (0, 0) (X) {$X$};
\node at (3, 0) (F) {$\mathcal{F}(X)$};
\node at (0, -1) (A) {$A$};
\draw[->] (X) -- node[above] {inclusion} (F);
\draw[->] (X) -- node[left] {$\phi$} (A);
...
\]
there exists a unique graded algebra homomorphism $\tilde{\psi}: \mathcal{F}(X) \to A$ such that $\tilde{\psi}|_{L} = \psi$, i.e.
\[
\begin{tikzpicture}
\node at (0, 0) (L) {$L$};
\node at (3, 0) (U) {$\mathcal{U}(L)$};
\node at (0, -1) (A) {$A$};
\draw[->] (X) -- node[above] {inclusion} (U);
\draw[->] (X) -- node[left] {$\psi$} (A);
...
\[
\mathfrak{L}_{X} = \bigoplus_{n \ge 1} \bigoplus_{\alpha \vDash n} \CC\operatorname{-spanning}\{\text{brackets of $x_{i_{1}}, \ldots, x_{i_{\ell}}$} \;|\; x_{i_{k}} \in X_{\alpha_{k}} \},
\]
...
\begin{proof}
Immediately we have (1)$\Rightarrow$(2). Assume that $\dim(H_n)=\dim(K_n) \text{ for all } n\geq 0,$ or equivalently $\mathcal{H}(H;t)=\mathcal{H}(K;t).$ By freeness, there exist graded sets $X$ and $Y$ generating $H$ and $K$, respectively. We define the Hilbert series of $X$ as $\mathcal{H}(X;t)=\sum_{i\geq 0}|X_i|t^i$. It follows from Prop. 3.2 that
...
We then have $|X_i|=|Y_i|$ for all $i\geq 0.$ Fix bijections $\varphi_i:X_i\to Y_i$ and extend to a bijection $\varphi:X\to Y$. With $\mathcal{L}(Y)$ the free Lie algebra on $Y$, the inclusion $Y\to \mathcal{L}(Y)$ induces a map $\Phi:X\to \mathcal{L}(Y).$ By freeness, $\Phi$ extends uniquely to a Lie algebra morphism $\hat{\Phi}:\mathcal{L}(X)\to \mathcal{L}(Y)$ which is a grading preserving isomorphism. We conclude by the Milnor-Moore theorem~\cite[Theorem 5.18]{MM65} and Theorem 1 that 
$$\mathcal{P}(H)\cong \mathcal{L}(X) \text{ and } \mathcal{P}(K)\cong \mathcal{L}(Y)$$. \eric{This version has a lot of overlap with Thm 3}
Therefore,
$$H\cong \mathcal{U}(\mathcal{L}(X))\cong \mathcal{U}(\mathcal{L}(Y))\cong K.$$
\end{proof}
