...
\begin{proof}
    ...
    Now given any $Y = \biguplus_{n \geq 1} \vec{Y}_n \in \OPG(K)$, we will construct an isomorphism $H \to K$ whose image on $X$ is $Y$ and respects ordering. Define $f_n:X_n \to Y_n$ with $f_n(x_n^i) = y_n^i$ for all $n \geq 1$, and extend to $f:X \to Y$. By freeness, $f$ extends to the desired isomorphism $\tilde{f}:H \to K$.
\end{proof}
