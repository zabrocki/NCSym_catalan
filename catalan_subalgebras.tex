...
\mike{inverse Euler transform is $\phi_{h,p}$ (cite OEIS for the conventions),
then Euler transform is $\phi_{p,h}$; INVERT is the transformation $\phi_{h,a}$, INVERTi is $\phi_{a,h}$;
there might not be a name for $\phi_{a,p}$ and $\phi_{p,a}$ but should check more carefully}
...
\]
so the $\vec{a}$ sequence is equal to $\vec{a} = (1,1,2,5,14,42, \ldots)$
(that is, $a_n$ is the $n-1^{st}$ Catalan number).
We can use Proposition~\ref{prop:sequences} to calculate the first few values of $\vec{p} = (1,1,3,8,25, \ldots)$ and it is known \cite{}\lucas{TODO: need to explain this, see Mike's comment below} that $p_n$ is the number of binary Lyndon words of length $2n$
with the same number of $0$s as $1$s.
\end{example}
\mike{That is Example 3.3 says $\phi_{h,a}(1,2,5,14,\ldots) = (1,1,2,5,14,42)$}
...
On the other hand, the Milnor--Moore theorem states that $H$ is isomorphic to the universal enveloping algebra of $\mathcal{P}(H)$.  
By the Poincare--Birkhoff--Witt theorem, for any fixed homogeneous basis $Y = \biguplus_{n \ge 0} Y^{(n)}$ of $\mathcal{P}(H)$, $h_{n}$ counts the multisets of $Y$ whose elements have degree summing (with repetition) to $n$.  Since $|Y^{(n)}| = p_{n}$ for all $n \ge 1$, the number of these mutlisets is exactly
\[
\sum_{\lambda \vdash n} \prod_{d \geq 1} \binom{p_d + m_d(\lambda) -1}{m_d(\lambda)},
\]
...
\begin{proposition}
\lucas{TODO: Convert this into a proof of Theorem~\ref{thm:OPGmaps} (i).  All that's needed is to link up the notation.} \felix{I will add it after the prop so we can decide after}
Let $H$ be a free graded connected cocommutative Hopf algebra,
$X \subset H$ a set of homogeneously ordered primitive generators
with $|X_n| = a_n$. For any $Y \in \OPG(H)$ there exists a unique
sequence $((\phi_n,\vec{B}_n))_{n=1}^{\infty}$ with
$\phi_n \in \GL_{a_n}(\mathbb{C})$ and $\vec{B}_n = (b_n^1,b_n^2, \ldots ,b_n^{a_n})$
with $b_n^i \in [\mathcal{P}(H),\mathcal{P}(H)]_n$, such that for
each ordered graded component $\vec{Y}_n = (y_n^1,y_n^2, \ldots ,y_n^{a_n})$ we have
\[
\vec{Y}_n = \phi_n \vec{X}_n + \vec{B}_n.
\]
\end{proposition}
\begin{proof}
    ...
    \begin{align*}
        \overline{y_n^i} &= \phi_n^{i1}\overline{x_n^1} + \phi_n^{i2}\overline{x_n^2} + \cdots + \phi_n^{ia_n}\overline{x_n^{a_n}}\\ 
        &= \overline{\phi_n^{i1}x_n^1 + \phi_n^{i2}x_n^2 + \cdots + \phi_n^{ia_n}x_n^{a_n}}\\
    \end{align*}
    for some coefficients $\phi_n^{ij}$. Therefore,
    \[
    y_n^i = \phi_n^{i1}x_n^1 + \phi_n^{i2}x_n^2 + \cdots + \phi_n^{ia_n}x_n^{a_n} + b_n^i
    \]
    for some $b_n^i \in [\mathcal{P}(H),\mathcal{P}(H)]$, and for $\phi_n = [\phi_n^{ij}] \in \GL_{a_n}(\mathbb{C})$ we have $\vec{Y}_n = \phi_n \vec{X}_n + \vec{B}_n$ as desired. 
\end{proof}

The uniqueness of $\vec{B}_n$ given by the previous proposition and the quotient map given by the definition of $\mathcal{U}(-)$ gives us that for each ordered primitive generating sets of $H$ there is a unique isomorphism $\phi=(\phi_n)_{n=1}^\infty$ between $\mathcal{U}(\mathfrak{L}(\vec{a}))$ and the Hopf algebra $H$. Hence, this gives us part (i) of Theorem \ref{thm:OPGmaps}.
