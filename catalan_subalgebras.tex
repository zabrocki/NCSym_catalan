...
This paper begins with a question about the Hopf algebra of symmetric functions in noncommutative variables, $\NCSym$~\cite{AgEtAl12, BHRZ05, GebSag01, HNT08, RS06, Wolf36}.  
...
We resolve Problem~\ref{problem:motivating_calatan_question} up to isomorphism by applying a recent result of Aliniaeifard and Thiem~\cite{AT20} and the Shirshov--Witt Theorem~\cite{S09, Witt56}.  
...
In the context of \FGCCHAs,~\cite[Theorem~4.2]{AT20} show that that the isomorphism class of an \FGCCHA $H$ can be fully determined by the \emph{dimension sequence}
\[
...
Moreover, the number of non-crossing set partitions is the Catalan numbers.

Using Aliniaefard and Thiem's result, $\mathbf{CQSym}$ is isomorphic to ever Hopf subalgebra of $\NCSym$ with graded dimensions equal to the Catalan nummbers, and in particular to $\{\mathbf{p}_{\pi} \;|\; \text{$\pi$ is noncrossing}\}$.  
...
However, in later sections these sequences come from an \FGCCHA $H$ and have an interpretation showing that they are non-negative integers.
...
\begin{remark} Although not necessarily obvious, an induction argument implies that if $\vec{a} \in \ZZ^{\ZZ_+}$ or $\vec{h} \in \ZZ^{\ZZ_+}$, then $\vec{p} \in \ZZ^{\ZZ_+}$.\lucas{citation?}
\end{remark}
...
Proposition \ref{prop:sequences} can be stated as biconditional theorem\lucas{don't love this phrasing} in the sense
...
If the sequence $\vec{a}$ consists entirely of non-negative integers,
