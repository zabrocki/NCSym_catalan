...
\begin{example}
	...
	Given two Hopf algebras $A$ and $B$ respectively associated with $\vec{a}$ sequences $\vec{a_1}=(0,1,0,0,\dots)$ and $\vec{a_2}=(1,0,0,0,\dots)$, let us see that the $\vec{h}$ sequences $\vec{h_1}$ associated to $\vec{a_1}$ is dominated by the $\vec{h}$ sequences $\vec{h_2}$ associated to $\vec{a_2}$ but corresponding $\vec{p}$ sequence $\vec{p_1}$ and $\vec{p_2}$ are not comparable. Following the computation of the $\vec{h}$ sequence from the $\vec{a}$ sequence given by 
