...
\end{proposition}
\begin{proof}
Let $\vec{a} = \veccard(X)$, $\vec{h} = \vecdim(H)$, and $\vec{p} = \vecdim(\mathcal{P}(H))$.  We will show that $\vec{h} = \phi_{a, h}(\vec{a})$ and $\vec{p} = \phi_{h, p}(\vec{p})$, from which the claim follows.  
