...
\end{thm}
\begin{proof}
    Let \(F(X)\) and \(\mathfrak{L}_{X}\) be the free algebra and free Lie algebra generated by X respectively. Define the map 
    \begin{align*}
        \begin{array}{rcl}
...
By Properties~\ref{prop:universalfree} and~\ref{prop:universalLie} we have the existence of unique graded algebra homomorphisms 
\begin{align*}
    \begin{array}{rcl}
    ...
    \end{array} 
\end{align*} 
We claim that \(\tilde{\phi} \circ \tilde{\psi} = id\). Recall that elements of \(F(X)\) can be written as 
\(g_{x_{i_{1}} }g_{x_{i_{2}} }\cdots g_{x_{i_{\ell}} }\) and that elements of \(\mathcal{U}(\mathfrak{L}_{X})\) can be written 
\(h_{x_{i_{1}} }h_{x_{i_{2}} }\cdots h_{x_{i_{k}} }\)  where each of the \(g_{x_{i_{\ell}} }\)  and \(h_{x_{i_{k}} }\) the product of formal generators 
\begin{align*}
...
\begin{lemma}~\cite[Lemma 11]{AT22}\eric{We can re-present their proof here, I was looking for different way to show it but havent been able to}.
    Let $H$ be a free graded connected cocommutative Hopf algebra. Then there exists a subset $X\subseteq \mathcal{P}(H)$ that freely generates $H$.
\end{lemma}

\begin{proposition}~\cite[Prop. 2.4]{F23}
    Let $L$ be a Lie algebra, and suppose $\mathcal{U}(L)$ is freely generated by $X\subseteq L$. Then $L$ is freely generated as a Lie algebra by $X$.
\end{proposition}
...
\begin{proof}
    ...
    To show uniqueness, suppose $g:L\to M$ is another Lie morphism such that $g|_X=f.$ The induced algebra map $G:\mathcal{U}(L)\to \mathcal{U}(M)$ has $G|_X=f$, and freeness of $\mathcal{U}(G)$ gives $G=F$, hence $g=\tilde{f}.$
\end{proof}

\begin{corollary}
    Let $H$ be a graded connected cocommutative Hopf algebra freely generated by $X$. Then $\mathcal{P}(H)\cong \mathfrak{L}(X).$
\end{corollary}
