...
\dim(H_{n}/H_{n}^{+2}) = |X^{(n)}|.
\]
There is a further relation between the quotient $H_{+}/H^{2}_{+}$ and generators of $H$, proven in \cite{F23}.

\begin{proposition}[{\cite[Proposition 2.2]{F23}}]
\label{prop:indecomposable generators}
If $A$ is a free graded connected algebra, and $X \subseteq A$ descends to a basis for $A_{+}/A^{2}_{+}$, then $X$ freely generates $A$.
\end{proposition}
...
Aguiar and Sottile use the Eulerian idempotent to produce primitive generators for $\mathcal{H}_{HO}$ in~\cite{aguiar2005cocommutative}, as do Novelli and Thibon for $\mathbf{CQSym}$ in~\cite[\S 5]{NT05}. 
Lauve and Mastnak use a related primitive idempotent to produce primitive generators for $\mathsf{NCSym}$ in~\cite{LM11}. 
...
We first show that the map which sends a the graded Hopf algebra isomorphism $\phi: \mathcal{U}(\mathfrak{L}(\vec{a})) \to H$ to the ordered set
\[
\phi(X_{\vec{a}}) = \biguplus_{n \ge 0} \big( \phi(x_{1}^{n}), \phi(x_{2}^{(n)}), \ldots, \phi(x_{a_{n}}^{(n)}) \big) \in \OPG(H)
\]
is a bijection.  The first step is to observe that this map is well-defined: since $\phi$ is an isomorphism the set $\phi(X_{\vec{a}})$ must freely generate $H$, and since $\phi$ is graded these generators are homogeneous.  
The injectivity of the assignment $\phi \mapsto \phi(X_{\vec{a}})$ follows from the fact that any two algebra morphisms which agree on a generating set must be equal.  
For surjectivity, consider an arbitrary $\vec{G} \in \OPG(H)$ and define $\phi_{\vec{G}}:\mathcal{U}(\mathfrak{L}(\vec{a})) \to H$ by algebraically extending the mapping of generators $\phi_{\vec{G}}: x_{i}^{(n)} \mapsto g_i^{(n)}$.
Then immediately $\phi_{\vec{G}}$ is an isomorphism and $\phi_{\vec{G}}(X_{\vec{a}}) = \vec{G} \in \OPG(H)$.

    We now prove that the set $\OPG(H)$ is in bijection with sequences $(A^{(1)}, A^{(2)}, \ldots)$ of matrix $A^{(n)} \in \mathrm{Mat}_{a_{n} \times p_{n}}(\CC)$ with the property that $(A^{(n)}_{i, j})_{1 \le i, j \le a_{n}}$ has full rank.  
    To this end, fix an element $G \in \OPG(H)$ and fix a homogeneous basis $\biguplus_{n \geq 1}\{v_1^{(n)},v_2^{(n)},\ldots,v_{p_n - a_n}^{(n)}\}$ of $[\mathcal{P}(H),\mathcal{P}(H)]$.  
    \lucas{TODO: define bracket subspace, add citation for this fact}
    Recall from Definition~\ref{defn:freelie} the free Lie algebra $\mathfrak{L}(X)$ on a graded set $X$, and that by \cite[Prop. 2.4]{F23}, we have $\mathcal{P}(H) \cong \mathfrak{L}(G)$.  
    Moreover, $\mathfrak{L}(G) = \CC G \oplus [\mathfrak{L}(G),\mathfrak{L}(G)]$, therefore
    \[
        \CC G \cong \mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)].
    \]
    We can then interpret an arbitrary choice of $S \in \OPG(H)$ as some homogeneous change of basis of the quotient space $\mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)]$ relative to $X$. \lucas{I'm not sure I see what this means.}
    Each homogeneous component $\vec{Y}^{(n)}$ then has the form
    \[
