...
In order to construct an explicit isomrphism, we first identify one such Hopf subalgebra.  The power-sum basis elements $\mathbf{p}_{\pi}$ of $\mathsf{NCSym}$ are also indexed also by non-crossing set partitions (see ~\cite{BHRZ05} for details on the Hopf structure on the $\{\mathbf{p}_{\pi}\}$ basis of \textsf{NCSym}).
