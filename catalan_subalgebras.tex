...
Foissy \cite{F12} has studied the category of free and cofree Hopf
algebras from a similar perspective and deduces that free
and cofree Hopf algebras are also determined by the sequences of
graded dimensions (Proposition 23).
Loday \cite{L08} develops the theory of a generalized bialgebra which includes the category of \FGCCHAs.  The results we develop for \FGCCHAs apply to many examples of combinatorial Hopf algebras that are studied in the literature fall into these categories and we hope that this will be a useful reference.
...
This paper collects some known results from the literature as a toolbox for what can be deduced about examples in the category of \FGCCHAs from associated sequences of integers.}
{\color{darkgreen}
At a more general level, properties of the category of \FGCCHAs (or its dual) have been considered in several references~\cite{AT22, Block85, FP24, LodayRonco10, MM65, Reutenauer-FreeLieAlgebras}, and key results appear in Loday's work on triples of operads, which include \FGCCHAs as an example~\cite[Section 4.1]{L08}.  
The particular ingredients of Aliniaefard and Thiem's result comes from Loday's generalization of the  Cartier--Milnor--Moore and Poincar\'{e}--Birkhoff--Witt theorems~\cite[Theorem 4.1.3]{L08}, which state that the structure of an \FGCCHA $H$, is determined by its Lie algebra of primitives, and the Poincar\'{e}--Birkhoff--Witt theorem then implies that \FGCCHAs are completely determined by the number of free generators and the dimension of the subspace of primitives.  Aliniaeifard and Thiem deduce their result from these properties.  
...
The Poincar\'{e}--Birkhoff--Witt Theorem~\cite[\S 0.1]{Reutenauer-FreeLieAlgebras}, \cite{Cartier2007}
...
The Cartier-Milnor-Moore theorem~\cite[Theorem 5.18]{MM65} states there is a Hopf algebra isomorphism
\[
\begin{array}{rcl}
...
The result is deduced from standard properties found in~\cite[Section 4.1]{L08}, so we omit a proof.  We demonstrate the utility of this result in Example~\ref{example:hopf_algebras_of_permutations}.
