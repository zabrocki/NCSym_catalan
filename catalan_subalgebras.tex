...
Theorem~\ref{prop:a sequence} states that an arbitrary nonnegative sequence $\vec{h}$
...
will hold.\mike{TODO: either indicated that the proof is further down or move the proof up}
%\end{remark}
%In light of 
...
\begin{proof} (of Proposition \ref{prop:sequences})
...
\mike{TODO: In the text it is stated as
Proposition \ref{prop:a sequence} and
is currently stated as a Theorem, the
labels and text need to be changed}
\label{prop:a sequence}
...
and by Proposition \ref{prop:a sequence} conclude
...
We conclude by giving an alternative sequence-based characterization of \FGCCHAs which follows as a Corollary of Theorem~\ref{thm:h sequence}.\lucas{We will fold this into a remark after theorem 2.}
...
\begin{proof}	
    ...
    Hence \(a_n \geq 0\) 
    for all \(n \geq 0\) by Proposition~\ref{prop:a sequence}.

    Supposing \(a_n \geq 0\) for all \(n \geq 0\), then by Proposition~\ref{prop:a sequence} we have that \(\vec{a}\)
...
\lucas{TODO: cut this example down on detail, add references to the \FGCCHA papers where this appears: Mike says it may appear in two, Bergeron and possibly one other.  If there are good connections, we can keep it after Theorem~\ref{thm:h sequence}}
