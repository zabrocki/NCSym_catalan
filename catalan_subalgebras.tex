...
\begin{example}
\lucas{TODO: Add SHORT explanation of the h, p, and a sequences of $\mathsf{NSym}$ with some citation.  Mike says ``Gelfand--Krobb--Lasoux--Leclerc--Retakh--Thibon, advances'' is a probably sufficient citation.}
We consider two well-known \FGCCHAs from the perspective of Proposition~\ref{prop:SequencesAsHopfStructures}.
\begin{enumerate}
\item $\mathrm{NSym}$

\item $\mathrm{CQSym}$\lucas{TODO: This moves down to Example~\ref{example:OPG}}
...
\begin{thm}
\lucas{TODO: This is actually a statement about ``$a$-sequences,'' let's change it to that and then mention that we can say it more complexly using $h$-sequences and/or $p$-sequences.}
...
We do an example for the Catalan \FGCCHA.  We take $\mathcal{U}(\mathfrak{L}(\vec{a}))$ to be generated by Dyck words (of one smaller degree) and $H$ to be the noncrossing subalgebra (in some basis) of $\mathsf{NCSym}$.  By (Lauve--Mastanek or Bergeron et.~al.), we have a set or primitive generators for $\mathsf{NCSym}$, which gives an isomorphism by mapping Dyck words to primitives
\end{example}
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    We now prove the second statement of the theorem. By \cite[F23]{Prop. 2.4}, for any $X \in \OPG(H)$ we have $\mathcal{P}(H) \cong \mathfrak{L}(X)$. Then, because $\mathfrak{L}(X) = \CC X \oplus [\mathfrak{L}(X),\mathfrak{L}(X)]$ \lucas{TODO: -define  $\mathfrak{L}(X)$?, add citation for this fact}we have
    \[
    ...
    \]
    descends to a basis for $\mathcal{P}(H)/[\mathcal{P}(H),\mathcal{P}(H)]$. It follows from \cite[F23]{Corollary 2.6}\lucas{TODO:fill in number} that $Z = \biguplus_{n \geq 1} \vec{Z}^{(n)} \in \OPG(H)$, and it is clear the correspondence between sequences $\left(A^{(n)}\right)_{n \geq 1}$ and $\OPG(H)$ is one to one.
\end{proof}
