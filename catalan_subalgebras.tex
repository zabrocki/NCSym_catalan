...
\lucas{Finish the rest of the proof in a similar fashion.  Nicolas will do.}.
...
\begin{proposition}
	...
	Following this we can construct the Hopf algebra associated to it.  \lucas{Change to constructive statement like in 6.2}
\end{proposition}

\begin{proof}
\lucas{Cross-reference with Proposition~\ref{prop:sequences}.  Nicolas will do.}
...
\[
\operatorname{pgs}(H) = \{\text{sets of homogeneous primitive generators of $H$}\}.
\]

\begin{proposition}
Describe how to construct $\operatorname{pgs}(H)$ here.
\end{proposition}



\begin{proposition}
Suppose that $A$ and $B$ are free graded connected cocommutative Hopf algebras with $\dim A_n = \dim B_n$ for all $n \geq 0$.  Then $A \cong B$.  Moreover, for any fixed $X \in \operatorname{pgs}(A)$, there exists a bijection
\[
\begin{array}{rcl}
\{\text{isomorphisms $A \to B$}\} &\leftrightarrow& \{\text{Graded bijections $X \to Y$ for $Y \in \operatorname{pgs}(B)$}\} \\
\phi & \mapsto & \phi(X) \\
\tilde{f} & \mapsfrom & f: X \to Y
\end{array}
\]
