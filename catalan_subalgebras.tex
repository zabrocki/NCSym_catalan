...
\begin{definition}
\label{def:FGCCHA}
An \FGCCHA is a graded cocommutative Hopf algebra $H$ which is freely generated by some graded set $X = \biguplus_{n \ge 1} X^{(n)}$, so that \textit{as algebras}
\[
H \cong \mathsf{T}(\CC X) 
\qquad\text{where}\qquad
\CC X = \bigoplus_{k \ge 0} \CC\operatorname{-span}\{x \in X^{(k)}\}.
\]
Under this isomorphism, the coproduct of $H$ corresponds to some graded algebra homomorphism $\Delta: \mathsf{T}(\CC X) \to \mathsf{T}(\CC X) \otimes \mathsf{T}(\CC X)$.
\end{definition}
...
\]
In general $\mathcal{P}(H)$ is a Lie algebra under the bracket operation $[x,y] = xy - yx$.  However it is not the case that $\mathcal{P}(H)$ maps to $\mathfrak{L}(X)$ under the isomorphism in Definition~\ref{def:FGCCHA}, as the generating set $X$ need not be primitive.  \lucas{TODO: is this clear?}
...
is the dual Hopf algebra to the quasisymmetric functions $\mathsf{QSym}$.\lucas{TODO: relevant?}
...
Given a graded Lie algebra $L = \bigoplus_{n \ge 1} L^{(n)}$, the universal enveloping algebra is the cocommutative Hopf algebra\lucas{TODO: redundant, and unclear what the coproduct is}
\[
...
The Cartier-Milnor-Moore theorem~\cite[see Theorem 5.18]{MM65} states there is a Hopf algebra isomorphism \lucas{why ``see''}
\[
\begin{array}{rcl}
...
Recall the free graded Lie algebra from Definition~\ref{def:freelie} and for each sequence $\vec{a} \in \NN^{\ZZ_{+}}$, let
\begin{equation}
\label{eq:La}
\mathfrak{L}(\vec{a}) = \mathfrak{L}(X) \qquad\text{for}\qquad X = \biguplus_{n \ge 1} \{x^{(n)}_{i} \;|\; 1 \le i \le a_{n}\}.
\end{equation}
\lucas{TODO: is this the right notation for our generating set?}
...
\end{array}.
\]
In particular, $\vec{h} \in \QQ^{\ZZ_+}$ is the dimension sequence of an \FGCCHA if and only if $\phi_{h, a}(\vec{h}) \in \NN^{\ZZ_+}$.  \lucas{Discuss: why $\mathbb{Q}$ here for $\vec{h}$}
\end{theorem}
...
\begin{example}\mike{TODO: discuss this one because $\phi_{ap}(\vec{a}) = (2, 2, 4, 7, 16, \ldots)$}\lucas{you are right}
\label{ex:OPG}
...
\cite[\S 3]{GERSTENHABER1991263},\cite[\S 4.5.2]{loday2013cyclic},\cite[\S 8.4]{Reutenauer-FreeLieAlgebras})
\[
...
\begin{proof}[Proof of Theorem \ref{thm:OPGmaps}]
    ...
    We now prove the second statement of the theorem. 
    Recall from Definition~\ref{defn:freelie} the free Lie algebra $\mathfrak{L}(X)$ on a graded set $X$.  
    By \cite[Prop. 2.4]{F23},
    ...
    \]
    \lucas{TODO: define bracket subspace, add citation for this fact}
...
\item For every surjective homomorphism $f: H \to K$, there exists a (non-unique) choice of ordered primitive generating sets $X \in \OPG(H)$ and $Y \in \OPG(K)$ for which $f = f^{X}_{Y}$ as in Equation~\eqref{eq:defaultsurjection}.\lucas{GROUP TODO: do we need "as in equation..."?}
