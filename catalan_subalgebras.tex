...
\begin{proof}
    ...
    \]
    Because $F|_X=f$ we have $F(x)\in M$, hence $\Delta_M\circ F(x)=1\otimes F(x)+F(x)\otimes 1$. Extending algebraically we see $F$ is a Hopf morphism. The primitive functor then gives the desired map $\tilde{f}=\mathcal{P}(F):L\to M$ which one can verify uniquely extends $f$.
\end{proof}
