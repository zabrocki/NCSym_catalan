...
We will make extensive use of explicit formulas relating the sequences $\vec{h}$, $\vec{p}$, and $\vec{a}$.  The following formula can be derived directly from Equation~\eqref{eq:gf_relation} using standard techniques.  The first part of (iii) below also appears in~\cite[Theorem 2.2]{KK95} as the ```generalized Witt's formula.''
