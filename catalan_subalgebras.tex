...
\]
Thus, $\mathsf{fcyc}$ is periodic of order $n$.\mike{I think it might be clearer if we say that for some words it might be period smaller than $n$, but for all words in general it is of period $n$}  Moreover, if $k$ is the minimal positive integer for which $\mathsf{cyc}^{k}(w) = w$, then the $\mathsf{fcyc}$ orbit of $(0, w)$---or $(i, w)$ for any $1 \le i \le \mathsf{deg}(w_{1})$---has size
\[
