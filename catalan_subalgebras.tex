...
For a graded set $X$, the free graded Lie algebra $\mathfrak{L}(X)$ is the smallest subspace of $\mathsf{T}(\CC X)$ that contains $X$ and is closed under the commutator bracket, $[x,y] = x \otimes y - y \otimes x$.
...
\mike{TODO: citation?} \felix{Let us agree on this one}
...
We will also make use of the \emph{Eulerian Idempotent} (see \cite[Section 1.4]{AL15},\cite{aguiar2005cocommutative} ,\cite{GERSTENHABER1991263},\cite[Section 4.5.2]{loday2013cyclic},\cite[Section 8.4]{Reutenauer-FreeLieAlgebras})\lucas{TODO: Add original citation or reference (text) for eulerian idempotent.  First place to look: Aguiar--Lauve's ``Adams operator'' paper.  N.B.: Mike had identified a citation for this in the google chat.} \felix{The first one suggested seems to cover the subject. I added them all}
\[
